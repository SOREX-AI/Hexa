...
"""Verify that engine Cargo manifests follow workspace manifest policy.

Checks:
- Crates inherit `[workspace.package]` metadata.
- Crates opt into `[lints] workspace = true`.
- Crate names follow the engine directory naming conventions.
...
MANIFEST_FEATURE_EXCEPTIONS = {
    "engine/code-mode/Cargo.toml": {"sandbox": ("v8/v8_enable_sandbox",)},
    "engine/v8-poc/Cargo.toml": {"sandbox": ("v8/v8_enable_sandbox",)},
}
...
def main() -> int:
    ...
    print(
        "Cargo manifests under engine must inherit workspace package metadata, "
    ...
    print(
        "Cargo only applies `engine/Cargo.toml` `[workspace.lints.clippy]` "
    ...
    print()
    print(
        "Package-name checks apply to `engine/<crate>/Cargo.toml` and "
        "`engine/utils/<crate>/Cargo.toml`."
    )
    print(
