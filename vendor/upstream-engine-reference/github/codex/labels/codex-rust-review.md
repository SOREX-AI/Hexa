...
- Each crate in the Cargo workspace in `engine` has a specific purpose: make a note if you believe new code is not introduced in the correct crate.
- When possible, try to keep the `core` crate as small as possible. Non-core but shared logic is often a good candidate for `engine/common`.
...
- If the nature of the change seems to have a visual component (which is often the case for changes to `engine/tui`), recommend including a screenshot or video to demonstrate the change, if appropriate.
