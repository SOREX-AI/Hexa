...
# Remote-env setup script for engine integration tests.
#
# Usage (source-only):
...
setup_remote_env() {
  ...
  codex_binary_path="${CARGO_TARGET_DIR:-${REPO_ROOT}/engine/target}/debug/codex"
  ...
  (
    cd "${REPO_ROOT}/engine"
    cargo build -p vendor/upstream-js-cli --bin codex
  )
