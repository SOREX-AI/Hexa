...
export class AppServerClient extends EventEmitter {
  ...
  async start(binaryPath: string, options: AppServerStartOptions): Promise<void> {
    ...
    // A fast app-server startup failure can close stdin while initialization
    // or a user-triggered retry is still writing. Treat the broken pipe as the
    // process failure it belongs to instead of crashing Electron's main process.
