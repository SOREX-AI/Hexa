...
const upstreamReferenceRoot = path.join(repoRoot, 'vendor', 'upstream-engine-reference');
if (existsSync(upstreamReferenceRoot)) {
  for (const relative of [
    ...
    if (!existsSync(path.join(upstreamReferenceRoot, relative))) errors.push(`missing complete upstream reference/${relative}`);
  }
  if (existsSync(path.join(upstreamReferenceRoot, '.git'))) errors.push('upstream reference contains nested Git metadata');
  if (existsSync(path.join(upstreamReferenceRoot, 'codex-rs'))) errors.push('upstream codex-rs was duplicated inside the inert reference tree');
  try {
    const source = JSON.parse(readFileSync(path.join(upstreamReferenceRoot, 'UPSTREAM_SOURCE.json'), 'utf8'));
    if (!/^[0-9a-f]{40}$/.test(source.revision) || !source.remote || !source.ref) errors.push('upstream reference provenance is incomplete');
  } catch {
    errors.push('upstream reference provenance is unreadable');
  }
}
...
if (!errors.length) {
  ...
  const appServerModels = readFileSync(path.join(engineRoot, 'app-server', 'src', 'models.rs'), 'utf8');
  if (!appServerModels.includes('.list_models(RefreshStrategy::Online, http_client_factory)')) {
    errors.push('app-server model/list does not refresh the live model catalog');
  }
  const stateMigrations = readFileSync(path.join(engineRoot, 'state', 'src', 'migrations.rs'), 'utf8');
  const stateRuntime = readFileSync(path.join(engineRoot, 'state', 'src', 'runtime.rs'), 'utf8');
  const stateSqlite = readFileSync(path.join(engineRoot, 'state', 'src', 'sqlite.rs'), 'utf8');
  const stateManifest = readFileSync(path.join(engineRoot, 'state', 'Cargo.toml'), 'utf8');
  if (!stateMigrations.includes('ignore_missing: true')) {
    errors.push('runtime SQLite migrations no longer tolerate databases written by a newer Hexa Engine');
  }
  if (!stateRuntime.includes('open_state_sqlite_tolerates_newer_applied_migrations')) {
    errors.push('runtime SQLite forward-schema compatibility regression test is missing');
  }
  if (!stateMigrations.includes('repair_line_ending_migration_checksums')
    || !stateSqlite.includes('repair_line_ending_migration_checksums(&pool, migrator).await?;')
    || !stateManifest.includes('sha2 = { workspace = true }')) {
    errors.push('runtime SQLite migrations are not protected against CRLF/LF checksum drift');
  }
  ...
  const tooltipTextPath = existsSync(path.join(engineRoot, 'tui', 'assets', 'tooltips.txt'))
    ? path.join(engineRoot, 'tui', 'assets', 'tooltips.txt')
    : path.join(engineRoot, 'tui', 'tooltips.txt');
  const tooltipText = readFileSync(tooltipTextPath, 'utf8');
  ...
  if (binaryManager.includes('refreshCachedSandboxSetup')) {
    errors.push('sandbox helper refresh can incorrectly mark a mixed-version runtime cache as current');
  }
  ...
  if (!appServerClient.includes("child.stdin.on('error'")) {
    errors.push('Hexa app-server stdin failures can still crash the Electron main process');
  }
