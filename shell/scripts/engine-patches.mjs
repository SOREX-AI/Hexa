...
import { applyHexaSqliteCompatibility } from './engine-sqlite-compatibility.mjs';
import { applyHexaModelCatalogRefresh } from './engine-model-catalog-refresh.mjs';
...
const sandboxReadinessSource = path.join('app-server', 'src', 'request_processors', 'windows_sandbox_processor.rs');
const sandboxReadinessAnchor = `    pub(crate) async fn windows_sandbox_readiness(
        &self,
    ) -> Result<WindowsSandboxReadinessResponse, JSONRPCErrorError> {
        Ok(determine_windows_sandbox_readiness(&self.config))
    }`;
const sandboxReadinessPatch = `    pub(crate) async fn windows_sandbox_readiness(
        &self,
    ) -> Result<WindowsSandboxReadinessResponse, JSONRPCErrorError> {
        // Sandbox setup persists the selected mode after this app-server has
        // started. Reload the user config so readiness does not keep reporting
        // the startup mode until the process is restarted.
        let config = self
            .config_manager
...
const sandboxRefreshLockAnchor = `    lock_sandbox_bin_dir(payload, &sandbox_group_sid)?;

    if refresh_only {
        log_line(`;
const sandboxRefreshLockPatch = `    // Elevated provisioning already protects the helper directory before the
    // unelevated refresh runs. Reapplying its protected DACL from the refresh
    // process fails with access denied and incorrectly turns a successful
    // provisioning run into a failed setup.
    if !refresh_only {
...
async function normalizeHexaDependencyAliases(engineRoot) {
  const changedFiles = new Set();
  const manifests = (await walk(engineRoot)).filter((file) => path.basename(file) === 'Cargo.toml');
  for (const manifest of manifests) {
    if (path.resolve(manifest) === path.resolve(path.join(engineRoot, 'Cargo.toml'))) continue;
    const cargo = await readFile(manifest, 'utf8');
    const aliases = [...cargo.matchAll(/^\s*(hexa-[A-Za-z0-9_-]+)\s*=/gm)].map((match) => match[1]);
    if (!aliases.length) continue;
    for (const file of await walk(path.dirname(manifest))) {
      if (path.extname(file) !== '.rs') continue;
      const text = await readFile(file, 'utf8');
      let next = text;
      for (const alias of aliases) {
        const suffix = alias.slice('hexa-'.length).replaceAll('-', '_');
        next = next.replaceAll(`codex_${suffix}::`, `hexa_${suffix}::`);
      }
      if (next !== text) {
        await writeFile(file, next);
...
async function stripSplitDaemonUpdaterReferences(engineRoot) {
  let filesChanged = 0;
  const pidStartPath = path.join(engineRoot, 'app-server-daemon', 'src', 'backend', 'pid_start.rs');
  if (existsSync(pidStartPath)) {
    const text = await readFile(pidStartPath, 'utf8');
    let next = text.replace('#[cfg(windows)]\nuse super::PidCommandKind;\n', '');
    next = next.replace(/\n            if matches!\(self\.command_kind, PidCommandKind::UpdateLoop\) \{[\s\S]*?\n            \}\n            let shutdown_file/, '\n            let shutdown_file');
    next = next.replace(/\n        #\[cfg\(windows\)\]\n        if matches!\(self\.command_kind, PidCommandKind::UpdateLoop\) \{[\s\S]*?\n        \}\n        drop\(reservation_lock\);/, '\n        drop(reservation_lock);');
    if (next !== text) {
      await writeFile(pidStartPath, next);
      filesChanged += 1;
    }
  }

  const pidTestsPath = path.join(engineRoot, 'app-server-daemon', 'src', 'backend', 'pid_tests.rs');
  if (existsSync(pidTestsPath)) {
    const text = await readFile(pidTestsPath, 'utf8');
    const next = text.replace(/\n#\[test\]\nfn update_loop_uses_hidden_app_server_subcommand\(\) \{[\s\S]*?\n\}\n\n#\[test\]\nfn app_server_remote_control_uses_runtime_flag/, '\n#[test]\nfn app_server_remote_control_uses_runtime_flag');
    if (next !== text) {
      await writeFile(pidTestsPath, next);
      filesChanged += 1;
    }
  }
  return filesChanged;
...
async function validateHexaPackageLayer(engineRoot) {
  ...
  const windowsSandboxProcessor = await readFile(path.join(engineRoot, sandboxReadinessSource), 'utf8');
  const windowsSandboxSetup = await readFile(path.join(engineRoot, sandboxSetupSource), 'utf8');

  for (const relativeDir of ['config', 'core', 'prompts', 'protocol']) {
    const sourceRoot = path.join(engineRoot, relativeDir);
    for (const file of await walk(sourceRoot)) {
      if (path.extname(file) !== '.rs') continue;
      const text = await readFile(file, 'utf8');
      if (text.includes('codex_execpolicy::')) {
        violations.push(`${path.relative(engineRoot, file)} still uses the pre-rebrand execpolicy crate alias`);
      }
    }
  }
  for (const file of await walk(path.join(engineRoot, 'hexa-mcp', 'src'))) {
    if (path.extname(file) !== '.rs') continue;
    const text = await readFile(file, 'utf8');
    if (text.includes('codex_exec_server::')) {
      violations.push(`${path.relative(engineRoot, file)} still uses the pre-rebrand exec-server crate alias`);
    }
  }
  ...
  if (!windowsSandboxProcessor.includes('Reload the user config so readiness does not keep reporting')) violations.push('live Windows sandbox readiness refresh');
  if (!windowsSandboxSetup.includes('Reapplying its protected DACL from the refresh')) violations.push('Windows sandbox refresh helper-directory handling');

  for (const file of await walk(path.join(engineRoot, 'app-server-daemon', 'src'))) {
    if (path.extname(file) !== '.rs') continue;
    const text = await readFile(file, 'utf8');
    if (text.includes('PidCommandKind::UpdateLoop') || text.includes('pid-update-loop')) {
      violations.push(`${path.relative(engineRoot, file)} still contains the disabled standalone updater mode`);
    }
  }
...
export async function applyHexaEnginePatches(engineRoot) {
  await applyHexaModelCatalogRefresh(engineRoot);
  ...
  await applyHexaSqliteCompatibility(engineRoot);
  // Upstream can add new uses of a dependency after Hexa has renamed its Cargo
  // key. Keep Rust paths synchronized with each consumer manifest, including
  // when migrating an engine that is already staged.
  let crateAliasFilesChanged = await normalizeHexaDependencyAliases(engineRoot);
  crateAliasFilesChanged += await replaceRustTreeLiterals(engineRoot, path.join('arg0', 'src'), [
    ['hexa_linux_sandbox_exe', 'codex_linux_sandbox_exe'],
  ]);
  crateAliasFilesChanged += await stripSplitDaemonUpdaterReferences(engineRoot);
  const sandboxReadiness = path.join(engineRoot, sandboxReadinessSource);
  let sandboxReadinessText = await readFile(sandboxReadiness, 'utf8');
  if (!sandboxReadinessText.includes('Reload the user config so readiness does not keep reporting')) {
    sandboxReadinessText = replaceRequired(
      sandboxReadinessText,
      sandboxReadinessAnchor,
      sandboxReadinessPatch,
      `${sandboxReadinessSource} changed around the readiness handler`,
    );
    await writeFile(sandboxReadiness, sandboxReadinessText);
  }
  ...
  // resolve_sid moved from this setup module into the shared sandbox crate.
  // Both layouts still expose it here, so anchor the retry beside the stable
  // group-SID wrapper rather than beside resolve_sid's former implementation.
  const sandboxRetryAnchor = 'pub fn resolve_sandbox_users_group_sid() -> Result<Vec<u8>> {';
...
pub fn resolve_sandbox_users_group_sid() -> Result<Vec<u8>> {`;
  ...
  if (!sandboxSetupText.includes('Reapplying its protected DACL from the refresh')) {
    sandboxSetupText = replaceRequired(
      sandboxSetupText,
      sandboxRefreshLockAnchor,
      sandboxRefreshLockPatch,
      `${sandboxSetupSource} changed around the refresh helper-directory lock`,
    );
  }
  if (!sandboxSetupText.includes('use sandbox_users::resolve_sid_after_provisioning;')) {
    // Do not include line endings in these anchors: a Windows checkout can
    // materialize upstream Rust sources with CRLF before the staged copy.
    const legacyImport = 'use sandbox_users::resolve_sid;';
    const currentImport = 'use sandbox_users::resolve_sandbox_users_group_sid;';
    sandboxSetupText = sandboxSetupText.includes(legacyImport)
      ? replaceRequired(
        sandboxSetupText,
        legacyImport,
        `${legacyImport}\nuse sandbox_users::resolve_sid_after_provisioning;`,
        `${sandboxSetupSource} changed around legacy SID imports`,
      )
      : replaceRequired(
        sandboxSetupText,
        currentImport,
    ...
    sandboxSetupText = replaceRequired(
      ...
      `${sandboxSetupSource} changed around post-provisioning SID lookup`,
    );
  }
  ...
  if (alreadyPackageRebranded) {
    ...
    return {
      bazelFilesChanged: 0,
      brandingFilesChanged: crateAliasFilesChanged,
      cliIdentity: true,
  ...
  let brandingFilesChanged = crateAliasFilesChanged;
  ...
  const tuiTooltipsSource = existsSync(path.join(engineRoot, 'tui', 'assets', 'tooltips.txt'))
    ? path.join('tui', 'assets', 'tooltips.txt')
    : path.join('tui', 'tooltips.txt');
  brandingFilesChanged += await replaceFileLiterals(engineRoot, tuiTooltipsSource, [
  ...
  for (const [relative, replacements] of [
    ...
    [existsSync(path.join(engineRoot, 'core', 'tests', 'fixtures', 'prompt_with_apply_patch_instructions.md'))
      ? path.join('core', 'tests', 'fixtures', 'prompt_with_apply_patch_instructions.md')
...

  {
    ...
    if (text.includes('pub(crate) enum RestartIfRunningOutcome')) {
      const restartTypesCfg = text.includes('#[cfg(any(unix, windows))]\n#[derive(Debug, Clone, Copy, PartialEq, Eq)]\npub(crate) enum RestartIfRunningOutcome')
    ...
    if (text.includes('pub async fn run_pid_update_loop(')) {
      const supportedPlatformCfg = text.includes('\n#[cfg(any(unix, windows))]\nfn ensure_supported_platform()')
        ? '\n#[cfg(any(unix, windows))]\nfn ensure_supported_platform()'
        : '\n#[cfg(unix)]\nfn ensure_supported_platform()';
      text = replaceBetweenRequired(
        text,
        'pub async fn run_pid_update_loop(',
        supportedPlatformCfg,
        '',
    ...
    if (text.includes('    pub(crate) async fn try_restart_if_running(')) {
      const restartMethodCfg = text.includes('    #[cfg(any(unix, windows))]\n    pub(crate) async fn try_restart_if_running(')
        ? '    #[cfg(any(unix, windows))]'
        : '    #[cfg(unix)]';
      text = replaceBetweenRequired(
        text,
    ...
    if (text.includes('fn restart_decision(')) {
      const restartDecisionCfg = text.includes('#[cfg(any(unix, windows))]\nfn restart_decision(')
        ? '#[cfg(any(unix, windows))]'
        : '#[cfg(unix)]';
      text = replaceBetweenRequired(
        text,
    ...
    if (text.includes('        let managed_codex_path = self.managed_codex_bin.display();')) {
      const managedVersionCfg = text.includes('\n    #[cfg(any(unix, windows))]\n    async fn managed_codex_version_best_effort')
        ? '\n    #[cfg(any(unix, windows))]\n    async fn managed_codex_version_best_effort'
        : '\n    #[cfg(unix)]\n    async fn managed_codex_version_best_effort';
      text = replaceBetweenRequired(
        text,
        '        let managed_codex_path = self.managed_codex_bin.display();',
        `${managedVersionCfg}(&self) -> Option<String> {`,
...
`;
  ...
  brandingFilesChanged += await replaceRustTreeLiterals(engineRoot, path.join('arg0', 'src'), [
    ['codex_linux_sandbox::', 'hexa_linux_sandbox::'],
    // Preserve the upstream compatibility field name; only the helper crate
    // and executable identities are Hexa-owned.
    ['hexa_linux_sandbox_exe', 'codex_linux_sandbox_exe'],
  ]);
  ...
  const finalTooltipText = await readFile(path.join(engineRoot, tuiTooltipsSource), 'utf8');
  ...
  const finalStateMigrations = await readFile(path.join(engineRoot, 'state', 'src', 'migrations.rs'), 'utf8');
  const finalStateRuntime = await readFile(path.join(engineRoot, 'state', 'src', 'runtime.rs'), 'utf8');
  const finalStateSqlite = await readFile(path.join(engineRoot, 'state', 'src', 'sqlite.rs'), 'utf8');
  const finalStateManifest = await readFile(path.join(engineRoot, 'state', 'Cargo.toml'), 'utf8');
  const violations = [];
  ...
  if (!finalStateMigrations.includes('ignore_missing: true') || !finalStateRuntime.includes('open_state_sqlite_tolerates_newer_applied_migrations')) violations.push('SQLite forward-schema compatibility');
  if (!finalStateMigrations.includes('repair_line_ending_migration_checksums') || !finalStateSqlite.includes('repair_line_ending_migration_checksums(&pool, migrator).await?;') || !finalStateManifest.includes('sha2 = { workspace = true }')) violations.push('SQLite cross-platform line-ending compatibility');
  ...
  brandingFilesChanged += await normalizeHexaDependencyAliases(engineRoot);
  await validateHexaPackageLayer(engineRoot);
