...
fn missing_host_error_limits_the_displayed_path_to_512_bytes() {
    let executable = "hexa-code-mode-host-does-not-exist";
...
async fn provider_returns_missing_host_error() {
    let provider = ProcessOwnedCodeModeSessionProvider::with_host_program(
        "hexa-code-mode-host-does-not-exist".into(),
    );
    ...
    assert!(error.contains("failed to spawn code-mode host hexa-code-mode-host-does-not-exist"));
}
