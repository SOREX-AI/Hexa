...
async fn pipes_stdin_and_stdout_through_socket() -> anyhow::Result<()> {
    ...
    let child_task = tokio::task::spawn_blocking(move || -> anyhow::Result<ChildOutput> {
        ...
        let mut child = Command::new(codex_utils_cargo_bin::cargo_bin("hexa-stdio-to-uds")?)
            .arg(&socket_path)
            .stdin(Stdio::from(stdin))
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .spawn()
            .context("failed to spawn hexa-stdio-to-uds")?;
        ...
        let status = loop {
            ...
            if Instant::now() >= deadline {
                ...
                anyhow::bail!(
                    "hexa-stdio-to-uds did not exit in time; server events: {:?}; stderr: {}",
                    server_events,
    ...
    assert!(
        ...
        "hexa-stdio-to-uds exited with {status}; server events: {:?}; stderr: {}",
