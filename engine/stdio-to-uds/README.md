...
```
codex --config mcp_servers.example={command="hexa-stdio-to-uds",args=["/tmp/mcp.sock"]}
```
...
As a workaround, this crate uses `hexa-uds`, which provides a cross-platform async UDS API backed by https://crates.io/crates/uds_windows on Windows.
