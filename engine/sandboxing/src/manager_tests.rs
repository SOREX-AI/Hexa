...
fn transform_linux_seccomp_request(
    hexa_linux_sandbox_exe: &std::path::Path,
    ...
    manager
        .transform(SandboxTransformRequest {
            ...
            network: None,
            sandbox_policy_cwd: &cwd_uri,
            hexa_linux_sandbox_exe: Some(hexa_linux_sandbox_exe),
            use_legacy_landlock: false,
            windows_sandbox_level: WindowsSandboxLevel::Disabled,
            windows_sandbox_private_desktop: false,
        })
...
fn transform_linux_seccomp_preserves_helper_path_in_arg0_when_available() {
    let hexa_linux_sandbox_exe = std::path::PathBuf::from("/tmp/hexa-linux-sandbox");
    let exec_request = transform_linux_seccomp_request(&hexa_linux_sandbox_exe);

    assert_eq!(
        exec_request.arg0,
        Some(hexa_linux_sandbox_exe.to_string_lossy().into_owned())
    );
}
...
fn transform_linux_seccomp_uses_helper_alias_when_launcher_is_not_helper_path() {
    let hexa_linux_sandbox_exe = std::path::PathBuf::from("/tmp/codex");
    let exec_request = transform_linux_seccomp_request(&hexa_linux_sandbox_exe);

    assert_eq!(exec_request.arg0, Some("hexa-linux-sandbox".to_string()));
}
