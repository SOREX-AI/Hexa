...
fn normalize_writable_root_for_sandbox(
    ...
    if !allow_symlinks && let Some(symlink) = nested_symlink_component(root.as_path()) {
        return Err(SeatbeltPreparationError::FileSystem(format!(
            "writable root {} contains symlink component {}; symlinked writable roots are not supported.\n\
             If this writable root is at or beneath HEXA_ENGINE_HOME and you trust its symlink targets, \
             set `allow_symlinked_codex_home = true` at the top level of `$HEXA_ENGINE_HOME/config.toml` \
             (normally `~/.hexashell/config.toml`) on the execution host, then restart Codex or its executor. \
             This opt-out trusts targets outside HEXA_ENGINE_HOME and targets changed between commands. \
    ...
    let normalized = if allow_symlinks {
        ...
        // outside HEXA_ENGINE_HOME and links changed by an earlier command.
