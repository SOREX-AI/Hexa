...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            "-DWRITABLE_ROOT_0_EXCLUDED_0=/.hexa".to_string(),
...
fn create_seatbelt_args_with_read_only_git_and_codex_subpaths() {
    ...
    // write to .hexa/config.toml in the vulnerable root.
    ...
    // Verify that .hexa/config.toml cannot be modified under the generated
    // Seatbelt policy.
...
fn seatbelt_protects_resolved_target_of_symlinked_metadata_directory() {
    ...
    let dot_codex = writable_root.join(".hexa");
...
fn create_seatbelt_args_block_first_time_dot_codex_creation_with_metadata_name_regex() {
    ...
    let dot_codex = repo_root.join(".hexa");
...
struct PopulatedTmp {
    ...
    /// ran `git commit` themselves, or modified .hexa/config.toml to
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".hexa")).expect("create .codex");
    fs::write(
        vulnerable_root.join(".hexa").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .hexa/config.toml");
    ...
    let dot_codex_canonical = vulnerable_root_canonical.join(".hexa");
