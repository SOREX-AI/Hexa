...
`hexa-network-proxy` is Codex's local network policy enforcement proxy. It runs:
...
`hexa-network-proxy` reads from Codex's merged `config.toml` (via `hexa-core` config loading).
...
# bundle env vars pointing at immutable public files under $HEXA_ENGINE_HOME/proxy/ so common HTTPS
...
```bash
cargo run -p hexa-network-proxy --
```
...
`hexa-network-proxy` can be embedded as a library with a thin API:
...
When `hexa-network-proxy` is embedded in managed Codex runtime, policy decisions emit structured
...
This section documents the protections implemented by `hexa-network-proxy`, and the boundaries of
