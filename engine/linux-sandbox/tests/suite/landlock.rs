...
fn hexa_linux_sandbox_exe() -> PathBuf {
    let sandbox_program = PathBuf::from(env!("CARGO_BIN_EXE_hexa-linux-sandbox"));
...
async fn run_cmd_result_with_permission_profile_for_cwd(
    ...
    let hexa_linux_sandbox_exe = Some(hexa_linux_sandbox_exe());
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let hexa_linux_sandbox_exe: Option<PathBuf> = Some(hexa_linux_sandbox_exe());
...
async fn sandbox_blocks_git_and_codex_writes_inside_writable_root() {
    ...
    let dot_codex = tmpdir.path().join(".hexa");
...
async fn sandbox_rejects_symlinked_synthetic_mount_registry() {
    ...
    let registry = temp.path().join(format!(
        "hexa-bwrap-synthetic-mount-targets-{effective_uid}"
    ));
...
registry="${{TMPDIR:-/tmp}}/hexa-bwrap-synthetic-mount-targets-$(id -u)"
if touch "$registry/forged-marker" 2>/dev/null; then
  exit 22
fi
redirected_registry='{redirected_tmp}'/hexa-bwrap-synthetic-mount-targets-$(id -u)
...
"#,
    ...
    assert!(!subdir.join(".hexa").exists());
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = hexa_linux_sandbox_exe()
        .parent()
...
async fn sandbox_starts_with_denied_tmp_without_exposing_registry() {
    ...
    let sandbox_helper = hexa_linux_sandbox_exe();
...
registry="$TMPDIR/hexa-bwrap-synthetic-mount-targets-$(id -u)"
test ! -e "$registry"
"#,
                ],
                cwd.clone(),
