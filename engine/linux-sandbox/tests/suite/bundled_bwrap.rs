...
use hexa_linux_sandbox::BUNDLED_BWRAP_DIGEST_VERIFICATION_FAILURE_EXIT_CODE;
...
fn bazel_build_rejects_tampered_bundled_bwrap() {
    ...
    let resources = package.path().join("hexa-resources");
    std::fs::create_dir(&resources).expect("package resource directory should be created");

    let sandbox_binary = package.path().join("hexa-linux-sandbox");
    std::fs::copy(env!("CARGO_BIN_EXE_hexa-linux-sandbox"), &sandbox_binary)
