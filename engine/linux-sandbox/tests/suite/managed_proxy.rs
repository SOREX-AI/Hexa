...
fn linux_sandbox_command(
    ...
    let mut cmd = Command::new(env!("CARGO_BIN_EXE_hexa-linux-sandbox"));
    cmd.args(args)
        .current_dir(cwd)
        .env_clear()
        .envs(env)
...
async fn unsupported_system_bwrap_falls_back_to_bundled_bwrap() {
    ...
    let sandbox_executable = tempdir.path().join("hexa-linux-sandbox");
    let original_executable = env!("CARGO_BIN_EXE_hexa-linux-sandbox");
    ...
    let resources_dir = tempdir.path().join("hexa-resources");
...
async fn managed_proxy_mode_routes_through_bridge_and_blocks_direct_egress() {
    ...
    let sandbox_helper_dir = std::path::Path::new(env!("CARGO_BIN_EXE_hexa-linux-sandbox"))
        .parent()
