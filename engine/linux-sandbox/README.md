...
- a `hexa-linux-sandbox` standalone executable for Linux that is bundled with the Hexa Engine
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `hexa-exec` CLI can check if its arg0 is `hexa-linux-sandbox` and, if so, execute as if it were `hexa-linux-sandbox`
  - this should also be true of the `hexa-engine` multitool CLI

On Linux, Hexa prefers the first `bwrap` found on `PATH`
...
the helper falls back to the bundled `hexa-resources/bwrap` binary shipped
with Hexa.
Hexa also surfaces a startup warning when `bwrap` is missing so users know it
is falling back to the bundled helper. Hexa surfaces the same startup warning
...
it cannot create the required user namespaces, so Hexa rejects sandboxed shell
...
- If `bwrap` is missing, the helper falls back to the bundled
  `hexa-resources/bwrap` path.
- If `bwrap` is missing, Hexa also surfaces a startup warning instead of
  printing directly from the sandbox helper.
- If bubblewrap cannot create user namespaces, Hexa surfaces a startup warning
  instead of waiting for a runtime sandbox failure.
- WSL2 uses the normal Linux bubblewrap path.
- WSL1 is not supported for bubblewrap sandboxing; Hexa rejects sandboxed
...
- When bubblewrap is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.hexa`) are re-applied as read-only via `--ro-bind`.
...
**Notes**
- The CLI surface is `hexa-engine sandbox`; the host OS selects the sandbox backend.
