...
pub async fn run_otlp_trace_receiver(
    ...
    let local_address = listener
        .local_addr()
        .context("failed to read OTLP trace receiver listen address")?;
    info!("hexa-code-mode-host OTLP trace receiver listening on http://{local_address}");
...
pub async fn run_otel_trace_listener(
    ...
    info!("hexa-code-mode-host OTEL trace websocket listening on ws://{local_address}");
    let router = Router::new()
