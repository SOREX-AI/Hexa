...
#[derive(Debug, Parser)]
#[command(
    name = "hexa-thread-manager-sample",
    about = "Run one Hexa Engine turn through ThreadManager and print mapped notifications as newline-delimited JSON."
)]
struct Args {
...
async fn run_main(arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    let local_runtime_paths = ExecServerRuntimePaths::from_optional_paths(
        ...
        config.hexa_linux_sandbox_exe.clone(),
    )?;
    ...
    let NewThread {
        ...
        .await
        .context("start Hexa Engine thread")?;
    ...
    turn_output?;
    shutdown_result.context("shut down Hexa Engine thread")?;

    Ok(())
}

fn new_config(model: Option<String>, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<Config> {
    let codex_home = find_codex_home().context("find Hexa Engine home")?;
...
async fn run_turn(thread: &CodexThread, thread_id: &str, prompt: String) -> anyhow::Result<()> {
    ...
    loop {
        let event = thread.next_event().await.context("read Hexa Engine event")?;
