...
fn bash_snapshot_filters_invalid_exports() -> Result<()> {
    let output = Command::new("/bin/bash")
        ...
        .env("VALID_NAME", "ok")
        .env("PWD", "/tmp/stale")
        .env("NEXTEST_BIN_EXE_hexa-write-config-schema", "/path/to/bin")
        .env("BAD-NAME", "broken")
        .output()?;
    ...
    assert!(!stdout.contains("NEXTEST_BIN_EXE_hexa-write-config-schema"));
...
fn zsh_snapshot_restores_tied_path() -> Result<()> {
    ...
    let readonly_restored = Command::new("/bin/zsh")
        .arg("-f")
        .arg("-c")
        .arg("set -e; . \"$1\"; export PATH='/hexa-path':\"$PATH\"; print -r -- \"$PATH\"")
        .arg("zsh")
        .arg(&snapshot_path)
        .env_clear()
        .env("PATH", "/usr/bin:/bin")
