...
pub struct Cli {
    ...
    /// Allow running Hexa Engine outside a Git repository.
    ...
    /// Do not load `$HEXA_ENGINE_HOME/config.toml`; auth still uses `HEXA_ENGINE_HOME`.
...
struct ResumeArgsRaw {
    ...
    // so "hexa-engine resume --last <prompt>" treats the positional as a prompt, not a session id.
