...
use hexa_exec_server::HttpResponseBodyStream;
...
async fn tool_catalog_cache_sanitizes_tools_and_tracks_environment_generation() {
    ...
    let cache_context = |environment: &Arc<hexa_exec_server::Environment>| {
        cache
            .context(
                "docs",
                &config,
...
fn mcp_init_error_display_prompts_for_login_when_auth_required() {
    ...
    let expected = format!(
        "The {server_name} MCP server is not logged in. Run `hexa-engine mcp login {server_name}`."
    );
...
fn mcp_init_error_display_identifies_oauth_reauthentication() {
    ...
    for (config, recovery_hint) in [
        (None, "Run `hexa-engine mcp login example`."),
        (
            Some(&executor_config),
...
async fn reconciliation_reuses_legacy_stdio_server_with_existing_protocol_marker() {
    let runtime_context = McpRuntimeContext::new(
        Arc::new(hexa_exec_server::EnvironmentManager::default_for_tests()),
        PathBuf::from("/tmp"),
    );
    let mut config = reusable_server_config("http://127.0.0.1:1");
