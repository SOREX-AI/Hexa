...
impl OAuthStoreLock {
    ...
    fn acquire_with_timeout(
        ...
        // authority. Those stores are HEXA_ENGINE_HOME-backed today: if HEXA_ENGINE_HOME is unset they use
        // the default home (`~/.hexashell`), and if an embedder has no local home/filesystem authority
...
pub(super) enum OAuthStoreLockFailure {
    #[error("failed to resolve HEXA_ENGINE_HOME for MCP OAuth {store} aggregate-store lock")]
    CodexHome {
        store: OAuthStore,
        #[source]
        source: io::Error,
