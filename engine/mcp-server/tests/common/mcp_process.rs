...
use hexa_mcp_server::CodexToolCallParam;
...
impl McpProcess {
    ...
    pub async fn new_with_env(
        ...
        let program = codex_utils_cargo_bin::cargo_bin("hexa-mcp-server")
            .context("should find binary for hexa-mcp-server")?;
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("hexa-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `hexa-mcp-server` child process.
        //
