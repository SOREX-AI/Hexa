...
fn migrated_quoted_hook_command(script_name: &str) -> String {
    let hook_path = Path::new("/repo/.hexa")
...
fn hook_command_paths_rewrite_to_target_hook_dir() {
    ...
    assert_eq!(
        rewrite_hook_command_cla(&plugin_script_command, Some(Path::new("/repo/.hexa")),),
...
fn hook_script_copy_keeps_existing_target_scripts() {
    ...
    let target_config_dir = root.path().join(".hexa");
