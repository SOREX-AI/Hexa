...
- From the Hexa Engine, run `hexa-engine execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
hexa-execpolicy check --rules path/to/policy.rules git status
```

- To opt into basename fallback for absolute program paths, pass `--resolve-host-executables`:

```bash
hexa-execpolicy check \
...
```bash
cargo run -p hexa-execpolicy -- check --rules path/to/policy.rules git status
```
