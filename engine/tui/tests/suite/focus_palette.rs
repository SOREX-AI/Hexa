...
impl PtyCodex {
    fn start(repo_root: &Path, codex_home: TempDir) -> Result<Self> {
        ...
        let codex = codex_utils_cargo_bin::cargo_bin("hexa-engine")
            .or_else(|_| codex_utils_cargo_bin::cargo_bin("hexa-tui"))?;
    ...
    fn wait_for_startup(&mut self) -> Result<()> {
        ...
        while Instant::now() < deadline {
            ...
            if self.palette_answered && self.screen_contains("Hexa Engine") {
                return Ok(());
            }
