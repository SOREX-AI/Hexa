...
pub(crate) struct RateLimitSnapshotDisplay {
    /// Canonical limit identifier (for example: `hexa-engine` or `codex_other`).
...
mod tests {
    ...
    fn non_codex_single_limit_renders_combined_row() {
        let now = Local::now();
        let primary = RateLimitSnapshotDisplay {
        ...
        let rows = match compose_rate_limit_data_many(&[primary, other], now) {
