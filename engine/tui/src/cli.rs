...
pub struct Cli {
    ...
    // Internal controls set by the top-level `hexa-engine resume` subcommand.
    // These are not exposed as user flags on the base `hexa-engine` command.
    #[clap(skip)]
    ...
    /// top-level `hexa-engine resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `hexa-engine fork` subcommand.
    // These are not exposed as user flags on the base `hexa-engine` command.
    #[clap(skip)]
    ...
    /// top-level `hexa-engine fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
