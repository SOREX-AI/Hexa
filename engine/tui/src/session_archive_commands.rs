//! Shared implementation for `hexa-engine archive`, `hexa-engine delete`, and `hexa-engine unarchive`.
//!
...
pub async fn run_session_archive_command(
    ...
    let codex_home = find_codex_home().wrap_err("failed to find Hexa Engine home")?;
    let mut app_server =
