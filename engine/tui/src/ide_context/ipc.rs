...
#[cfg(any(unix, windows))]
const TUI_SOURCE_CLIENT_ID: &str = "hexa-tui";
#[cfg(any(unix, windows))]
const OPEN_IDE_HINT: &str =
    "Open this project in VS Code or Cursor with the Hexa-compatible extension active.";
#[cfg(any(unix, windows))]
const IDE_DID_NOT_PROVIDE_CONTEXT_HINT: &str = "The IDE extension did not provide context.";
#[cfg(any(unix, windows))]
const KEEP_TRYING_HINT: &str = "Hexa Engine will keep trying on future messages.";
    ...
    pub(crate) fn user_facing_hint(&self) -> String {
        match self {
            ...
            IdeContextError::Send(_) => {
                "Hexa Engine could not request IDE context. Try /ide again.".to_string()
            }
            IdeContextError::Read(_) | IdeContextError::InvalidResponse(_) => {
                "Hexa Engine could not read IDE context. Try /ide again.".to_string()
            }
        }
    }
    ...
    pub(crate) fn prompt_skip_hint(&self) -> String {
        match self {
            ...
            IdeContextError::Read(error) if error.kind() == std::io::ErrorKind::TimedOut => {
                "Hexa Engine timed out waiting for IDE context. It will keep trying on future messages."
                    .to_string()
            }
            IdeContextError::RequestFailed(error) if error == "client-disconnected" => {
                hint_with_retry("The IDE connection changed while Hexa Engine was requesting context.")
            }
            ...
            IdeContextError::Send(_) => {
                hint_with_retry("Hexa Engine lost the IDE connection while requesting context.")
            }
            IdeContextError::InvalidResponse(_) => {
                hint_with_retry("Hexa Engine received an unexpected IDE context response.")
            }
            IdeContextError::RequestFailed(_) => hint_with_retry(IDE_DID_NOT_PROVIDE_CONTEXT_HINT),
            IdeContextError::Read(_) => hint_with_retry("Hexa Engine could not read IDE context."),
        }
    }
...
mod tests {
    ...
    fn primary_ipc_socket_path_uses_codex_home() {
        let codex_home = Path::new("/home/test/.hexashell");
