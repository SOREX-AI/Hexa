...
//! | `HEXA_ENGINE_HOME` | `OnceLock<Option<PathBuf>>` | Root for custom `.tmTheme` discovery |
//!
...
static HEXA_ENGINE_HOME: OnceLock<Option<PathBuf>> = OnceLock::new();
...
pub(crate) fn set_theme_override(
    ...
    let codex_home_set_ok = HEXA_ENGINE_HOME.set(codex_home.clone()).is_ok();
...
pub(crate) fn validate_theme_name(name: Option<&str>, codex_home: Option<&Path>) -> Option<String> {
    ...
    let custom_theme_path_display = codex_home
        ...
        .unwrap_or_else(|| format!("$HEXA_ENGINE_HOME/themes/{name}.tmTheme"));
...
fn resolve_theme_with_override(name: Option<&str>, codex_home: Option<&Path>) -> Theme {
    ...
    if let Some(name) = name {
        ...
        // 2. Try loading {HEXA_ENGINE_HOME}/themes/{name}.tmTheme from disk.
        if let Some(home) = codex_home
...
fn build_default_theme() -> Theme {
    ...
    let codex_home = HEXA_ENGINE_HOME
        .get()
...
pub(crate) fn configured_theme_name() -> String {
    ...
    if let Some(Some(name)) = THEME_OVERRIDE.get() {
        ...
        if let Some(Some(home)) = HEXA_ENGINE_HOME.get()
...
/// `.tmTheme` file under `{HEXA_ENGINE_HOME}/themes/`.
