...
async fn remote_project_trust_guards_thread_start_and_preserves_repository_decisions() -> Result<()>
{
    ...
    std::fs::create_dir(project_cwd.join(".hexa"))?;
    ...
    std::fs::write(
        project_cwd.join(".hexa/config.toml"),
    ...
    std::fs::remove_file(project_cwd.join(".hexa/config.toml"))?;
    std::fs::remove_dir(project_cwd.join(".hexa"))?;
