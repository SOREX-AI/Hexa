...
const PACKAGE_METADATA_FILENAME: &str = "hexa-package.json";
const PATH_DIRNAME: &str = "hexa-path";
const RELEASES_DIRNAME: &str = "releases";
const RESOURCES_DIRNAME: &str = "hexa-resources";
...
/// Version metadata recorded in a bundled Hexa runtime package.
#[derive(Clone, Debug, Deserialize, Eq, PartialEq)]
pub struct HexaPackageManifest {
    pub version: Version,
}
...
pub struct InstallContext {
    ...
    pub package_layout: Option<HexaPackageLayout>,
}
...
pub enum InstallMethod {
    Standalone {
        ...
        /// such as
        /// `~/.hexashell/packages/standalone/releases/0.111.0-x86_64-unknown-linux-musl`.
        /// Package-layout installs use the package root that contains `bin/`,
        /// `hexa-resources/`, and `hexa-path/`.
    ...
    /// A Hexa binary launched through the npm-managed `hexa-engine` shim.
    Npm,
    /// A Hexa binary launched through the bun-managed `hexa-engine` shim.
    Bun,
    /// A Hexa binary launched through the pnpm-managed `hexa-engine` shim.
    Pnpm,
    /// A Hexa binary launched through the Vite+-managed `hexa-engine` shim.
    VitePlus,
    /// A Hexa binary that appears to come from a Homebrew install prefix.
    Brew,
    /// Any other execution environment.
    ///
    /// This commonly covers `cargo run`, app-bundled Hexa binaries, custom
    /// internal launchers, and tests that execute Hexa from an arbitrary path.
    Other,
}
...
impl InstallContext {
    ...
    fn from_exe_with_codex_home(
        ...
        let package_layout = current_exe.and_then(HexaPackageLayout::from_exe);
    ...
    pub fn package_manifest(&self) -> Option<HexaPackageManifest> {
    ...
    pub fn code_mode_host_program(&self) -> PathBuf {
        // prefer the one packed under hexa-resources
        ...
        match exe_dir.file_name() {
            ...
            Some(name) if name == OsStr::new("MacOS") => {
                ...
                if contents.file_name()? != OsStr::new("Contents")
                    || bundle.file_name()? != OsStr::new("HexaCLI.app")
                {
                    return None;
                }
...
fn install_method_from_exe(
    exe_path: &Path,
    codex_home: Option<&Path>,
    package_layout: Option<&HexaPackageLayout>,
    is_macos: bool,
) -> InstallMethod {
...
mod tests {
    ...
    const TEST_RESOURCE_NAME: &str = "hexa-test-helper";
...
"#,
        ...
        let package_layout = HexaPackageLayout {
    ...
    fn standalone_package_layout_keeps_standalone_install_method() -> std::io::Result<()> {
        ...
        assert_eq!(
            ...
            InstallContext {
                ...
                package_layout: Some(HexaPackageLayout {
                    package_dir: canonical_package_dir,
