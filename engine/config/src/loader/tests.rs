...
"#,
    ...
    let expected_warnings = vec![format!(
        "Ignoring deprecated managed config file at {}; HEXA_ENGINE_HOME/managed_config.toml is no longer supported on Windows. Use %ProgramData%\\OpenAI\\Codex\\requirements.toml for enforced settings or config.toml for defaults.",
