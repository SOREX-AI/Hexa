...
///   `%ProgramData%\Hexa\Engine\requirements.toml` (Windows)
...
///   `%ProgramData%\Hexa\Engine\config.toml` (Windows)
/// - cloud     enterprise-managed cloud config bundle fragments
/// - user      `${HEXA_ENGINE_HOME}/config.toml`
/// - profile   `${HEXA_ENGINE_HOME}/<name>.config.toml`, when selected
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.hexa/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.hexa/config.toml` (loaded but disabled when untrusted)
...
pub async fn load_config_layers_state(
    ...
    let packaged_defaults_layer = if let Some(file) = &overrides.packaged_defaults_path {
        ...
        let config = toml::from_str(raw_toml).map_err(|error| {
            io::Error::new(
                io::ErrorKind::InvalidData,
                format!("invalid embedded packaged defaults; this is a Hexa Engine build error: {error}"),
            )
        })?;
...
fn windows_codex_system_dir() -> PathBuf {
    ...
    program_data.join("Hexa").join("Engine")
}
...
impl ProjectTrustContext {
    ...
    fn root_checkout_hooks_folder_for_dir(&self, dir: &AbsolutePathBuf) -> Option<AbsolutePathBuf> {
        ...
        Some(repo_root.join(relative_dir).join(".hexa"))
    }
}
...
async fn discover_project_layers(
    ...
    for dir in dirs {
        let dot_codex_abs = dir.join(".hexa");
    ...
    fn windows_system_requirements_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_requirements_toml_file()
                .expect("requirements.toml path")
                .as_path()
                .ends_with(Path::new("Hexa").join("Engine").join("requirements.toml"))
        );
    }
    ...
    fn windows_system_config_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_config_toml_file()
                .expect("config.toml path")
                .as_path()
                .ends_with(Path::new("Hexa").join("Engine").join("config.toml"))
        );
    }
}
