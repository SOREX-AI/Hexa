...
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in HEXA_ENGINE_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in HEXA_ENGINE_HOME.
    Auto,
...
pub enum OAuthCredentialsStoreMode {
    ...
    #[default]
    Auto,
    /// HEXA_ENGINE_HOME/.credentials.json
...
pub struct Tui {
    ...
    /// Use `/theme` in the TUI or see `$HEXA_ENGINE_HOME/themes` for custom themes.
    #[serde(default)]
    pub theme: Option<String>,

    /// Pet id to preselect in the terminal pet picker.
    ///
    /// Custom pet ids resolve against HEXA_ENGINE_HOME/pets/<pet-id>/pet.json.
    #[serde(default)]
