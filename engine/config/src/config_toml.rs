...
/// Base config deserialized from ~/.hexashell/config.toml.
...
pub struct ConfigToml {
    ...
    /// Allow macOS sandbox writable roots at or beneath HEXA_ENGINE_HOME to traverse
    ...
    /// they change between commands or lie outside HEXA_ENGINE_HOME.
    ...
    /// Defaults to `$HEXA_SQLITE_HOME` when set. Otherwise uses `$HEXA_ENGINE_HOME`.
    ...
    /// Defaults to `$HEXA_ENGINE_HOME/log`.
    ...
    /// directories for `.hexa` folders. Defaults to [".git"] when unset.
    #[serde(default)]
