...
impl ConfigLayerEntry {
    ...
    // Get the `.hexa/` folder associated with this config layer, if any.
    ...
    /// Returns the `.hexa/` folder that should be used for hook declarations.
    ///
...
impl ConfigLayerStack {
    ...
    /// the base `$HEXA_ENGINE_HOME/config.toml` layer because the active layer is the
