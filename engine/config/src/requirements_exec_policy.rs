...
use hexa_execpolicy::RuleRef;
use hexa_execpolicy::rule::PatternToken;
use hexa_execpolicy::rule::PrefixPattern;
use hexa_execpolicy::rule::PrefixRule;
...
pub enum RequirementsExecPolicyParseError {
    ...
    #[error(
        "rules prefix_rule at index {rule_index} has decision 'allow', which is not permitted in requirements.toml: Hexa Engine merges these rules with other config and uses the most restrictive result (use 'prompt' or 'forbidden')"
    )]
...
impl RequirementsExecPolicyToml {
    ...
    /// representation used by `hexa-execpolicy`.
