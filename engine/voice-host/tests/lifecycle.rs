...
fn spawn() -> Result<Child> {
    Ok(Command::new(cargo_bin("hexa-voice-host")?)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .kill_on_drop(true)
...
async fn build_commit() -> Result<String> {
    let output = timeout(
        DEADLINE,
        Command::new(cargo_bin("hexa-voice-host")?)
            .arg("--build-commit")
            .kill_on_drop(true)
            .output(),
    )
...
fn install_helper(root: &Path) -> Result<CodexPackageLayout> {
    ...
    let helper_dir = root.join("hexa-resources/voice/bin");
    fs::create_dir_all(&bin)?;
    fs::create_dir_all(&helper_dir)?;
    fs::write(root.join("hexa-package.json"), "{}")?;
    let app = bin.join(if cfg!(windows) { "codex.exe" } else { "codex" });
    fs::write(&app, [])?;
    let source = cargo_bin("hexa-voice-host")?;
