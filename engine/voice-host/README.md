...
`hexa-voice-host` establishes the inherited-pipe lifecycle for the proposed
...
`hexa-resources/voice/bin/hexa-voice-host[.exe]`, filters the child environment,
and owns process cleanup through `hexa-utils-pty`. Its runtime must remain alive
...
`CODEX_TEST_VOICE_RUNTIME=/absolute/prepared/runtime just test -p hexa-voice-host --test packaged_runtime --run-ignored all`.
