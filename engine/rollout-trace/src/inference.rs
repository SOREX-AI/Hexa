...
///
/// A Hexa Engine turn can create multiple attempts when auth recovery retries the
...
impl InferenceTraceContext {
    ...
    /// Builds an enabled context for all upstream attempts made by one Hexa Engine turn.
    pub fn enabled(
        writer: Arc<TraceWriter>,
        thread_id: AgentThreadId,
        codex_turn_id: CodexTurnId,
