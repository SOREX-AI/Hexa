...
`hexa-tools` is the shared support crate for building, adapting, and executing
model-visible tools outside `hexa-core`.
...
compatibility-sensitive orchestration in `hexa-core` until the surrounding
...
- code-mode compatibility shims that do not depend on `hexa-core`
- other narrowly scoped host utilities that multiple crates need

The corresponding non-goals are just as important:

- do not move `hexa-core` orchestration here prematurely
...
1. Keep extension-owned executable-tool authoring in `hexa-extension-api`.
2. Move host-side planning/adaptation helpers here when they no longer need to
   stay coupled to `hexa-core`.
3. Leave compatibility-sensitive adapters in `hexa-core` while downstream
...
`hexa-core`, that is a sign to revisit the extraction boundary before adding
more here.
