...
Higher-level request policy layered on `hexa-http-client` without any Codex/OpenAI API awareness.
...
- Re-exports the low-level HTTP types temporarily so consumers can migrate to `hexa-http-client` incrementally.
