...
fn recognizes_windows_executor_plugin_cache_root() {
    ...
    let script = PathUri::parse(
        "file:///C:/Users/user/.hexashell/plugins/cache/openai-primary-runtime/presentations/0.1.29/skills/presentations/container_tools/mark_artifact_operation_started.mjs",
    )
...
fn only_emits_safe_normalized_relative_script_paths() {
    ...
    assert_eq!(
        normalized_relative_script_path(Path::new(
            "/home/user/.hexashell/plugins/cache/openai-curated/sample/scripts/run.py"
        )),
        None
    );
}
