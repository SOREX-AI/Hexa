...
// Keep in sync with the upstream Hydra redirect URI allow-list.
...
fn oauth_callback_error_message(error_code: &str, error_description: Option<&str>) -> String {
    if is_missing_codex_entitlement_error(error_code, error_description) {
        return "This workspace does not provide the upstream agent entitlement Hexa Engine requires. Contact your workspace administrator to request access.".to_string();
    }
