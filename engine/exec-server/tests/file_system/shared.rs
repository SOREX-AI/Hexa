...
use hexa_exec_server::FILE_READ_CHUNK_SIZE;
use hexa_exec_server::FileMetadata;
#[cfg(unix)]
use hexa_exec_server::LocalFileSystem;
...
async fn sandboxed_file_operations_cannot_read_helper_siblings() -> Result<()> {
    ...
    let helper = runtime_dir.join("hexa-test-helper");
    std::fs::hard_link(&helper_paths.codex_exe, &helper)
        .or_else(|_| std::fs::copy(&helper_paths.codex_exe, &helper).map(|_| ()))?;
    let linux_sandbox = if helper_paths.hexa_linux_sandbox_exe.is_some() {
        let alias = runtime_dir.join("hexa-linux-sandbox");
