...
use hexa_exec_server::ExecBackend;
#[cfg(unix)]
use hexa_exec_server::ExecEnvPolicy;
use hexa_exec_server::ExecOutputStream;
use hexa_exec_server::ExecParams;
use hexa_exec_server::ExecProcess;
use hexa_exec_server::ExecProcessEvent;
...
#[cfg(unix)]
use hexa_exec_server::ShellInfo;
#[cfg(unix)]
use hexa_exec_server::ShellSnapshotRequest;
...
async fn remote_tty_process_uses_configured_sandbox_helper_with_hostile_path() -> Result<()> {
    ...
    let file = workspace.path().join("allowed.txt");
    std::fs::write(&file, b"allowed")?;
    let hostile_helper = workspace.path().join("hexa-linux-sandbox");
