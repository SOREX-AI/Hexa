...
`hexa-exec-server` is the library backing `codex exec-server`, a small
...
filesystem operations and `hexa-linux-sandbox`.

## Transport

The server speaks the exec-specific `hexa-exec-server-protocol` message
...
It uses the standard Codex ChatGPT sign-in state; run `hexa-engine login` first when
...
- `arg0`: optional argv0 override forwarded to `hexa-utils-pty`.
