...
pub struct ExecServerRuntimePaths {
    /// Stable path to the Hexa Engine executable used to launch hidden helper modes.
    ...
    pub hexa_linux_sandbox_exe: Option<AbsolutePathBuf>,
}
...
impl ExecServerRuntimePaths {
    pub fn from_optional_paths(
        ...
        let codex_self_exe = codex_self_exe.ok_or_else(|| {
            std::io::Error::new(
                ...
                "Hexa Engine executable path is not configured",
            )
        })?;
        Self::new(codex_self_exe, hexa_linux_sandbox_exe)
    }
    ...
    pub fn new(
        ...
        Ok(Self {
            ...
            hexa_linux_sandbox_exe: hexa_linux_sandbox_exe.map(absolute_path).transpose()?,
        })
    }
}
