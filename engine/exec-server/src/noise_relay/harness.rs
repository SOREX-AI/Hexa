            ...
            tokio::select! {
                maybe_message = outgoing_rx.recv(), if pending_outbound.is_none() && !force_incoming && !pong_deadline_expired => {
                    ...
                    let request_trace = match message {
                        hexa_exec_server_protocol::JSONRPCMessage::Request(request) => request.trace,
                        hexa_exec_server_protocol::JSONRPCMessage::Notification(_)
                        | hexa_exec_server_protocol::JSONRPCMessage::Response(_)
                        | hexa_exec_server_protocol::JSONRPCMessage::Error(_) => None,
                    };
