...
use hexa_exec_server_protocol::EnvironmentConfigLayer;
use hexa_exec_server_protocol::EnvironmentConfigLayerStack;
use hexa_exec_server_protocol::EnvironmentConfigReadParams;
use hexa_exec_server_protocol::EnvironmentConfigReadResponse;
...
pub(crate) async fn read_environment_config(
    ...
    let codex_home = find_codex_home().map_err(|error| {
        ReadEnvironmentConfigError::Internal(format!("failed to find Hexa Engine home: {error}"))
    })?;
