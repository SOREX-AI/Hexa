        ...
        sandbox_manager
            .transform_for_direct_spawn(SandboxDirectSpawnTransformRequest {
                ...
                transform: SandboxTransformRequest {
                    ...
                    hexa_linux_sandbox_exe: self.runtime_paths.hexa_linux_sandbox_exe.as_deref(),
...
fn helper_read_roots(runtime_paths: &ExecServerRuntimePaths) -> Vec<AbsolutePathBuf> {
    ...
    if let Some(path) = &runtime_paths.hexa_linux_sandbox_exe
        && !roots.contains(path)
    {
...
mod tests {
    ...
    fn helper_permissions_preserve_existing_writes() {
        ...
        let runtime_paths =
            ExecServerRuntimePaths::new(codex_self_exe, /*hexa_linux_sandbox_exe*/ None)
    ...
    fn helper_permissions_include_only_linux_sandbox_alias_executable() {
        ...
        let hexa_linux_sandbox_exe = root.path().join("aliases").join("hexa-linux-sandbox");
        let runtime_paths =
            ExecServerRuntimePaths::new(codex_self_exe, Some(hexa_linux_sandbox_exe))
                .expect("runtime paths");
        let cwd = AbsolutePathBuf::from_absolute_path(std::env::temp_dir().as_path())
            .expect("absolute cwd");
        let mut policy = restricted_policy(Vec::new());
