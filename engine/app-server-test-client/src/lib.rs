...
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $HEXA_ENGINE_HOME/config.toml to get a trace URL!";
...
struct Cli {
    ...
    /// spawning `hexa-app-server` as a child process.
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `hexa-app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/codex-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `hexa-app-server --listen`.
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let codex_bin = codex_bin.unwrap_or_else(|| PathBuf::from("hexa-engine"));
...
fn serve(codex_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    ...
    println!("started hexa-app-server");
...
impl CodexClient {
    ...
    fn spawn_stdio_with_env(
        ...
        let stdin = codex_app_server
            .stdin
            .take()
            .context("hexa-app-server stdin unavailable")?;
        let stdout = codex_app_server
            .stdout
            .take()
            .context("hexa-app-server stdout unavailable")?;
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to hexa-app-server")?;
                    return Ok(());
                }
                bail!("hexa-app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from hexa-app-server")?;
                if bytes == 0 {
                    bail!("hexa-app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl Drop for CodexClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[hexa-app-server exited: {status}]");
            return;
        }
