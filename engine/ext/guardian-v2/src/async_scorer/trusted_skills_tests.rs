...
fn renders_verified_skill_paths() {
    assert_eq!(
        rendered_paths(vec!["/home/user/.hexashell/skills/demo/SKILL.md".to_owned()]),
        vec!["/home/user/.hexashell/skills/demo/SKILL.md"],
    );
}
...
fn bounds_escaped_skill_paths_without_corrupting_json_or_utf8() {
    let paths = (0..MAX_TRUSTED_SKILLS)
        .map(|index| {
            format!(
                "/home/user/.hexashell/skills/{index:03}/{}SKILL.md",
                "\u{0001}é".repeat(80)
            )
        })
        .collect::<Vec<_>>();
...
fn preserves_multiple_invoked_skill_paths() {
    assert_eq!(
        rendered_paths(vec![
            "/home/user/.hexashell/skills/first/SKILL.md".to_owned(),
            "/home/user/.hexashell/skills/second/SKILL.md".to_owned(),
        ]),
        vec![
            "/home/user/.hexashell/skills/first/SKILL.md",
            "/home/user/.hexashell/skills/second/SKILL.md",
        ],
    );
}
...
fn invoked_skill_paths_are_deduplicated_and_bounded() {
    ...
    for index in 0..MAX_TRUSTED_SKILLS.saturating_mul(2) {
        let path = format!("/home/user/.hexashell/skills/{index:03}/SKILL.md");
    ...
    assert_eq!(
        ...
        (0..MAX_TRUSTED_SKILLS)
            .map(|index| format!("/home/user/.hexashell/skills/{index:03}/SKILL.md"))
            .collect::<Vec<_>>()
    );
