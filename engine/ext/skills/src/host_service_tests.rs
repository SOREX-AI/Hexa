...
async fn skills_for_cwd_loads_repo_and_user_roots_with_local_fs() {
    let codex_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let repo_dot_codex = cwd.path().join(".hexa");
    ...
    let other_file_system: Arc<dyn hexa_exec_server::ExecutorFileSystem> =
        Arc::new(hexa_exec_server::LocalFileSystem::unsandboxed());
