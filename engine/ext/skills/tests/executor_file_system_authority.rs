...
use hexa_exec_server::CapabilityRootDiscovery;
use hexa_exec_server::CapabilityTextFile;
use hexa_exec_server::CopyOptions;
use hexa_exec_server::CreateDirectoryOptions;
use hexa_exec_server::DiscoveredSkillFiles;
use hexa_exec_server::EnvironmentManager;
use hexa_exec_server::ExecutorCapabilityDiscoveryCache;
use hexa_exec_server::ExecutorCapabilityDiscoverySnapshot;
use hexa_exec_server::ExecutorFileSystem;
use hexa_exec_server::ExecutorFileSystemFuture;
use hexa_exec_server::FileMetadata;
use hexa_exec_server::FileSystemReadStream;
use hexa_exec_server::FileSystemSandboxContext;
use hexa_exec_server::GetMetadataOptions;
use hexa_exec_server::ReadDirectoryEntry;
use hexa_exec_server::ReadFileOptions;
use hexa_exec_server::RemoveOptions;
use hexa_exec_server::WalkEntry;
use hexa_exec_server::WalkEntryKind;
use hexa_exec_server::WalkOptions;
use hexa_exec_server::WalkOutcome;
use hexa_exec_server::WriteFileOptions;
...
async fn skill_loading_and_reads_use_the_supplied_executor_file_system() {
    let test_root =
        std::env::temp_dir().join(format!("hexa-executor-skill-fs-{}", std::process::id()));
    let project_folder = AbsolutePathBuf::from_absolute_path_checked(test_root.join(".hexa"))
...
async fn direct_executor_discovery_preserves_hidden_nested_and_probed_metadata() {
    let id = NEXT_TEST_ROOT_ID.fetch_add(1, Ordering::Relaxed);
    let test_root = std::env::temp_dir().join(format!(
        "hexa-executor-skill-discovery-{}-{id}",
        std::process::id()
    ));
