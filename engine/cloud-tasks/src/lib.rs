...
async fn init_backend(user_agent_suffix: &str) -> anyhow::Result<BackendContext> {
    ...
    let auth = match auth {
        ...
        None => {
            eprintln!(
                "Not signed in. Please run 'hexa-engine login' to sign in with ChatGPT, then re-run 'hexa-engine cloud'."
            );
            std::process::exit(1);
        }
    };
...
async fn resolve_environment_id(ctx: &BackendContext, requested: &str) -> anyhow::Result<String> {
    ...
    match label_matches.as_slice() {
        [] => Err(anyhow!(
            "environment '{trimmed}' not found; run `hexa-engine cloud` to list available environments"
        )),
        ...
        [first, rest @ ..] => {
            ...
            if rest.iter().all(|row| row.id == *first_id) {
                ...
                Err(anyhow!(
                    "environment label '{trimmed}' is ambiguous; run `hexa-engine cloud` to pick the desired environment id"
                ))
            }
        }
    }
...
async fn run_list_command(args: crate::cli::ListCommand) -> anyhow::Result<()> {
    ...
    if let Some(cursor) = page.cursor {
        let command = format!("hexa-engine cloud list --cursor='{cursor}'");
        if colorize {
            println!(
...
/// Entry point for the `hexa-engine cloud` subcommand.
pub async fn run_main(cli: Cli, _hexa_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
            ...
            Some(()) = redraw_rx.recv() => {
                ...
                if let Some(page) = app.new_task.as_mut() {
                    ...
                    if page.composer.is_in_paste_burst() {
                        let _ = frame_tx
                            .send(Instant::now() + hexa_tui::ComposerInput::recommended_flush_delay());
                    }
                }
                        ...
                        if let Some(page) = app.new_task.as_mut() {
                            ...
                            match key.code {
                                ...
                                _ => {
                                    ...
                                    } else if let hexa_tui::ComposerAction::Submitted(text) =
                                    ...
                                    if page.composer.is_in_paste_burst() {
                                        let _ = frame_tx.send(
                                            Instant::now()
                                                + hexa_tui::ComposerInput::recommended_flush_delay(),
                                        );
                                    }
