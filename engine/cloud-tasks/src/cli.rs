...
pub enum Command {
    /// Submit a new Hexa cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a Hexa cloud task.
    Status(StatusCommand),
    /// List Hexa cloud tasks.
    List(ListCommand),
    /// Apply the diff for a Hexa cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a Hexa cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Hexa cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `hexa-engine cloud` to browse).
    ...
    /// Git branch to run in Hexa cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Hexa cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Hexa cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Hexa cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
