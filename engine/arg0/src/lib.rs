...
#[cfg(unix)]
use hexa_exec_server::CODEX_ARG0_EXEC_HELPER_ARG1;
use hexa_exec_server::CODEX_FS_HELPER_ARG1;
...
#[cfg(unix)]
const EXECVE_WRAPPER_ARG0: &str = "hexa-execve-wrapper";
...
pub struct Arg0DispatchPaths {
    ...
    /// instead of the real Hexa Engine.
        ...
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                ...
                match runtime.block_on(codex_apply_patch::apply_patch_with_options(
                    ...
                    &mut stdout,
                    &mut stderr,
                    hexa_exec_server::LOCAL_FS.as_ref(),
                    /*sandbox*/ None,
                )) {
                    Ok(_) => 0,
                    Err(_) => 1,
...
/// While we want to deploy the Hexa Engine as a single executable for simplicity,
...
/// `hexa-linux-sandbox` we *directly* execute
/// [`hexa_linux_sandbox::run_main`] (which never returns). Otherwise we:
///
/// 1.  Load `.env` values from `~/.hexashell/.env` before creating any threads.
...
///     `hexa-linux-sandbox` helper path (falling back to the current
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    let paths = Arg0DispatchPaths {
        ...
        hexa_linux_sandbox_exe: if cfg!(target_os = "linux") {
...
fn linux_sandbox_exe_path(
    ...
    // Prefer the `hexa-linux-sandbox` alias when available so callers can
    // re-exec through a path whose basename still triggers arg0 dispatch on
    // bubblewrap builds that do not support `--argv0`.
    path_entry_guard
        .and_then(|path_entry| path_entry.paths().hexa_linux_sandbox_exe.clone())
        .or(current_exe)
}
...
/// Load env vars from ~/.hexashell/.env.
///
...
fn prepare_path_entry_for_codex_aliases(
    ...
    // Use a HEXA_ENGINE_HOME-scoped temp root to avoid cluttering the top-level directory.
...
mod tests {
    ...
    fn package_path_test_fixture() -> anyhow::Result<PackagePathTestFixture> {
        ...
        let path_dir = package_dir.join("hexa-path");
    ...
    fn windows_batch_alias_preserves_unicode_executable_paths() -> anyhow::Result<()> {
        ...
        let alias_directory = profile.join(".hexa").join("tmp").join("arg0");
    ...
    #[test]
    fn linux_sandbox_exe_path_prefers_hexa_linux_sandbox_alias() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let lock_file = create_lock(temp_dir.path())?;
        let alias_path = temp_dir.path().join("hexa-linux-sandbox");
        let path_entry = Arg0PathEntryGuard::new(
            temp_dir,
            lock_file,
            Arg0DispatchPaths {
                codex_self_exe: Some(PathBuf::from("/usr/bin/codex")),
                hexa_linux_sandbox_exe: Some(alias_path.clone()),
