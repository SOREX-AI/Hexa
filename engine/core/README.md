...
`bazel test //engine/core:core-all-wine-exec-test`.
...
Note that `hexa-core` makes some assumptions about certain helper utilities being available in the environment. Currently, this support matrix is:
...
pointer file), the resolved `gitdir:` target, and `.hexa` read-only.
...
Expects the binary containing `hexa-core` to run the equivalent of `codex sandbox` when `arg0` is `hexa-linux-sandbox`. See the `hexa-arg0` crate for details.
...
`bwrap` is missing, it falls back to the bundled `hexa-resources/bwrap`
...
Expects the binary containing `hexa-core` to simulate the virtual
`apply_patch` CLI when `arg1` is `--codex-run-as-apply-patch`. See the
`hexa-arg0` crate for details.
