...
async fn apply_role_preserves_unspecified_keys() {
    ...
    .await;
    config.codex_linux_sandbox_exe = Some(PathBuf::from("/tmp/hexa-linux-sandbox"));
    config.main_execve_wrapper_exe = Some(PathBuf::from("/tmp/hexa-execve-wrapper"));
    let role_path = write_role_config(
        &home,
    ...
    assert_eq!(
        ...
        Some(PathBuf::from("/tmp/hexa-linux-sandbox"))
    );
    assert_eq!(
        config.main_execve_wrapper_exe,
        Some(PathBuf::from("/tmp/hexa-execve-wrapper"))
    );
...
"#,
    ...
    let snapshot = skills_service
        .snapshot_for_config(&skills_input, Some(Arc::clone(&hexa_exec_server::LOCAL_FS)))
        .await;
