...
use hexa_execpolicy::prefix_rule_migration;
    ...
    async fn spawn_internal(args: SessionSpawnArgs) -> CodexResult<(Arc<Self>, SessionIo)> {
        ...
        let exec_policy = if crate::guardian::is_basic_session_source(&session_source) {
            let managed_policy = config
                ...
                .exec_policy
                .as_deref()
                .map_or_else(hexa_execpolicy::Policy::empty, |policy| {
                    policy.as_ref().clone()
                });
...
impl Session {
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &hexa_execpolicy::Policy,
        permission_profile: &PermissionProfile,
                    ...
                    .filter(|model| *model != curr)
                {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect Hexa Engine performance."
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) async fn request_permissions_for_environment(
        ...
        let Ok(native_environment_cwd) = environment.cwd.to_abs_path() else {
            warn!(
                cwd = %environment.cwd,
                "request_permissions requires a cwd native to the Hexa Engine host"
            );
            return Some(RequestPermissionsResponse {
                permissions: RequestPermissionProfile::default(),
                scope: PermissionGrantScope::Turn,
