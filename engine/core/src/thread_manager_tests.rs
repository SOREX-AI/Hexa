...
async fn spawn_internal_session_preserves_parent_lineage_without_forking_history() {
    ...
    let mut managed_exec_policy = hexa_execpolicy::Policy::empty();
    managed_exec_policy
        .add_prefix_rule(&["rm".to_string()], hexa_execpolicy::Decision::Forbidden)
        .expect("add managed execution restriction");
    let mut requirements = config.config_layer_stack.requirements().clone();
    requirements.exec_policy = Some(codex_config::Sourced::new(
        hexa_execpolicy::RequirementsExecPolicy::new(managed_exec_policy),
    ...
    reviewer_environment.config =
        EnvironmentConfigState::Ready(codex_protocol::protocol::EnvironmentConfig {
            ...
            exec_policy: Some(hexa_execpolicy::RequirementsExecPolicy::new(
                hexa_execpolicy::Policy::empty(),
            )),
            mcp_policy: None,
            network_policy: None,
