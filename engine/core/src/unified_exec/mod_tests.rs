...
use hexa_exec_server::ExecProcess;
use hexa_exec_server::ExecProcessEventReceiver;
use hexa_exec_server::ExecProcessFuture;
use hexa_exec_server::ProcessId;
use hexa_exec_server::ProcessSignal;
use hexa_exec_server::ReadResponse;
use hexa_exec_server::StartedExecProcess;
use hexa_exec_server::WriteResponse;
use hexa_exec_server::WriteStatus;
...
impl BlockingTerminateExecProcess {
    async fn read(&self) -> Result<ReadResponse, hexa_exec_server::ExecServerError> {
        Ok(ReadResponse {
            chunks: Vec::new(),
            next_seq: 1,
            exited: false,
    ...
    async fn write(&self) -> Result<WriteResponse, hexa_exec_server::ExecServerError> {
        Ok(WriteResponse {
            status: WriteStatus::Accepted,
        })
    }

    async fn terminate(&self) -> Result<(), hexa_exec_server::ExecServerError> {
...
async fn completed_pipe_commands_preserve_exit_code() -> anyhow::Result<()> {
    ...
    let environment = hexa_exec_server::Environment::default_for_tests();
