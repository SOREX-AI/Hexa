...
invalid = ["#,
    ...
    let user_layer = layers
        ...
        .expect("expected a user layer even when HEXA_ENGINE_HOME/config.toml is ignored");
    assert_eq!(
        user_layer.config,
...
async fn ignore_project_config_skips_project_discovery() -> std::io::Result<()> {
    ...
    .await?;
    let project_config_dir = workspace.join(".hexa");
...
async fn returns_packaged_defaults_when_other_layers_are_missing() {
    ...
    let user_layer = layers
        .get_active_user_layer()
        .expect("expected a user layer even when HEXA_ENGINE_HOME/config.toml does not exist");
...
"#
    ...
    let expected_log_dir =
        AbsolutePathBuf::resolve_path_against_base("~/.hexashell/logs", &codex_home);
    ...
    assert_eq!(
        ...
        Err(ConstraintError::InvalidValue {
            ...
            requirement_source: RequirementSource::MdmManagedPreferences {
                domain: "com.hexa.engine".to_string(),
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".hexa")).await?;
    tokio::fs::create_dir_all(project_root.join(".hexa")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".hexa").join(CONFIG_TOML_FILE),
        r#"foo = "root"
"#,
    )
    .await?;
    tokio::fs::write(
        nested.join(".hexa").join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;
    ...
    assert_eq!(project_layers[0].as_path(), nested.join(".hexa").as_path());
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".hexa").as_path()
    );
...
-> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(".hexa")).await?;
    tokio::fs::create_dir_all(worktree_child.join(".hexa")).await?;
    ...
    .await?;
    write_project_hook_config(
        &repo_child.join(".hexa"),
        Some("repo-child"),
        "echo repo child hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_root.join(".hexa"),
        Some("worktree-root"),
        "echo worktree root hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_child.join(".hexa"),
        Some("worktree-child"),
...
async fn forged_linked_worktree_does_not_inherit_repo_trust() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(attacker_root.join(".hexa")).await?;
    tokio::fs::write(
    ...
    .await?;
    tokio::fs::write(
        attacker_root.join(".hexa").join(CONFIG_TOML_FILE),
        r#"foo = "attacker"
"#,
    )
    .await?;
...
async fn malformed_untrusted_linked_worktree_does_not_read_root_hooks() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(".hexa")).await?;
    tokio::fs::create_dir_all(repo_root.join(".hexa")).await?;
    write_linked_worktree_pointer(&repo_root, &worktree_root).await?;
    tokio::fs::write(worktree_root.join(".hexa").join(CONFIG_TOML_FILE), "foo =").await?;
    tokio::fs::write(repo_root.join(".hexa").join(CONFIG_TOML_FILE), [0xff]).await?;
...
async fn nested_project_root_markers_do_not_redirect_regular_repo_hooks() -> std::io::Result<()> {
    ...
    .await?;
    write_project_hook_config(
        &project_root.join(".hexa"),
        /*foo*/ None,
        "echo project root hook",
    )
    .await?;
    write_project_hook_config(&nested.join(".hexa"), /*foo*/ None, "echo nested hook").await?;
    ...
    assert_eq!(
        project_layers[0].hooks_config_folder(),
        Some(AbsolutePathBuf::from_absolute_path(nested.join(".hexa"))?)
    );
    assert_eq!(
        project_layers[1].hooks_config_folder(),
        Some(AbsolutePathBuf::from_absolute_path(
...
"#;
    tokio::fs::write(project_root.join(".hexa").join(CONFIG_TOML_FILE), root_cfg).await?;
    tokio::fs::write(nested.join(".hexa").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".hexa").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(nested.join(".hexa").join("child.txt"), "child instructions").await?;
...
async fn project_layer_is_added_when_dot_codex_exists_without_config_toml() -> std::io::Result<()> {
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_codex_folder: AbsolutePathBuf::from_absolute_path(project_root.join(".hexa"))?,
        },
        TomlValue::Table(toml::map::Map::new()),
    );
...
async fn codex_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    ...
    let codex_home = home_dir.join(".hexa");
    tokio::fs::create_dir_all(&codex_home).await?;
    tokio::fs::write(
        codex_home.join(CONFIG_TOML_FILE),
        r#"foo = "user"
...
async fn codex_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    ...
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_codex = project_root.join(".hexa");
    let nested_dot_codex = nested.join(".hexa");
...
async fn project_layer_ignores_unsupported_config_keys() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let dot_codex = project_root.join(".hexa");
    ...
    // `.hexa` folder. The malformed profile value below would fail typed path
...
"#,
    ...
    // still use the project-local `.hexa` folder as its relative-path base.
    assert_eq!(
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".hexa")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".hexa").join(CONFIG_TOML_FILE), "foo =").await?;
...
mod requirements_exec_policy_tests {
    ...
    use codex_utils_absolute_path::AbsolutePathBuf;
    use hexa_execpolicy::Decision;
    use hexa_execpolicy::Evaluation;
    use hexa_execpolicy::RuleMatch;
    use pretty_assertions::assert_eq;
    use std::path::Path;
    use tempfile::tempdir;
    use toml::Value as TomlValue;
