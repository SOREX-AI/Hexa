...
async fn restricted_read_implicitly_allows_helper_executables() -> std::io::Result<()> {
    ...
    let codex_home = temp_dir.path().join(".hexa");
    let zsh_path = temp_dir.path().join("runtime").join("zsh");
    let arg0_root = codex_home.join("tmp").join("arg0");
    let allowed_arg0_dir = arg0_root.join("hexa-arg0-session");
    let sibling_arg0_dir = arg0_root.join("hexa-arg0-other-session");
    let execve_wrapper = allowed_arg0_dir.join("hexa-execve-wrapper");
