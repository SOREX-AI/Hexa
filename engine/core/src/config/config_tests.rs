...
-> std::io::Result<()> {
    ...
    for root in [&cwd, &runtime_root, &profile_root] {
        ...
        std::fs::create_dir_all(root.join(".hexa"))?;
    }
                        ...
                        filesystem: Some(FilesystemPermissionsToml {
                            ...
                            entries: BTreeMap::from([(
                                ...
                                FilesystemPermissionToml::Scoped(BTreeMap::from([
                                    ...
                                    (".hexa".to_string(), FileSystemAccessMode::Read),
                                ])),
                            )]),
                        }),
                        network: None,
    ...
    for root in [cwd_abs, runtime_root_abs, profile_root_abs.clone()] {
        ...
        assert!(
            !policy.can_write_local_path_with_cwd(&root.join(".hexa"), cwd.as_path()),
...
async fn empty_config_defaults_to_builtin_profile_for_trusted_project() -> std::io::Result<()> {
    ...
    if cfg!(target_os = "windows") {
        ...
        assert!(
            !policy.can_write_local_path_with_cwd(&cwd.path().join(".hexa"), cwd.path()),
...
-> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    let extra_root = TempDir::new()?;
    for subpath in [".git", ".agents", ".hexa"] {
...
async fn rebuild_preserving_session_layers_refreshes_requirements() -> std::io::Result<()> {
    ...
    let project_dot_codex =
        AbsolutePathBuf::resolve_path_against_base("project/.hexa", codex_home.path());
...
"#,
    ...
    let project_config_dir = workspace.path().join(".hexa");
...
"#
    ...
    let standalone_agents_dir = repo_root.path().join(".hexa").join("agents");
...
fn sqlite_home_env_conflict_reports_an_override() -> std::io::Result<()> {
    ...
    assert_eq!(
        warnings,
        vec![format!(
            "Environment value for `$HEXA_SQLITE_HOME` is overridden by the required `sqlite_home` value {required:?} from {}.",
            RequirementSource::Unknown
        )]
    );
