...
use hexa_exec_server::ExecutorFileSystem;
use hexa_exec_server::LOCAL_FS;
use hexa_exec_server::ReadFileOptions;
...
pub struct Config {
    ...
    /// event.
    ///
    /// Example `~/.hexashell/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "Hexa"]
    /// ```
    ///
    ...
    ///          https://github.com/openai/codex/blob/main/engine/rmcp-client/src/oauth.rs#L2
    /// file: HEXA_ENGINE_HOME/.credentials.json
    ...
    /// Directory containing all Codex state (defaults to `~/.hexashell` but can be
    /// overridden by the `HEXA_ENGINE_HOME` environment variable).
    pub codex_home: AbsolutePathBuf,

    /// Resolved configuration shared by all Codex SQLite databases.
    pub sqlite: codex_state::SqliteConfig,

    /// Directory where Codex writes log files (defaults to `$HEXA_ENGINE_HOME/log`).
    pub log_dir: PathBuf,

    /// Settings that govern if and what will be written to `~/.hexashell/history.jsonl`.
    pub history: History,
    ...
    /// Path to the `hexa-linux-sandbox` executable. This must be set if
    /// [`codex_sandboxing::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `hexa-linux-sandbox`.
    pub hexa_linux_sandbox_exe: Option<PathBuf>,

    /// Path to the `hexa-execve-wrapper` executable used for shell
...
impl Config {
    ...
    pub(crate) fn to_mcp_config_with_loaded_plugins(
        ...
        McpConfig {
            ...
            hexa_linux_sandbox_exe: self.hexa_linux_sandbox_exe.clone(),
    ...
    /// in [ConfigToml], such as `cwd`, `codex_self_exe`, `hexa_linux_sandbox_exe`, and
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .hexa/ folders.
...
/// Patch `HEXA_ENGINE_HOME/config.toml` project state to set trust level.
/// Use with caution.
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        // HEXA_SQLITE_HOME is an explicit runtime boundary supplied by the
        // desktop shell. Let it override a stale user-level sqlite_home from a
        // source checkout or previous installation, while still honoring an
        // exact managed requirement when an administrator has locked the path.
        let sqlite_home = if config_layer_stack.requirements().sqlite_home.is_some() {
            cfg.sqlite_home
                .as_ref()
                .cloned()
            ...
            sqlite_home_env
                .or_else(|| cfg.sqlite_home.as_ref().cloned())
                .unwrap_or_else(|| codex_home.clone())
        };
            ...
            && constrained_approval_policy.value() == AskForApproval::Never
        {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "`approval_policy = \"never\"` cannot be used because requirements do not allow `sandbox_mode = \"danger-full-access\"`; Hexa Engine would fall back to read-only permissions with approvals disabled. Choose an `approval_policy` based on what you need, such as `on-request`, or choose an allowed sandbox mode.",
            ));
        }
        if permission_profile_was_constrained {
...
/// specified by the `HEXA_ENGINE_HOME` environment variable. If not set, defaults to
/// `~/.hexashell`.
///
/// - If `HEXA_ENGINE_HOME` is set, the value must exist and be a directory. The
///   value will be canonicalized and this function will Err otherwise.
/// - If `HEXA_ENGINE_HOME` is not set, this function does not verify that the
///   directory exists.
