...
impl ModelClient {
    ...
    /// Responses Lite is an OpenAI transport dialect. Model metadata can be
    ...
    fn build_responses_request(
        ...
        // Responses Lite encodes tools and base instructions as developer-role
        // input items. Local OpenAI-compatible servers commonly translate every
        // such item into a system message, while model templates such as Qwen's
        // require exactly one leading system message. Keep local providers on
        // the standard Responses request shape so tools stay in `tools` and all
        // instructions can be represented by one leading instruction block.
        let (mut instructions, tools) = if use_responses_lite {
...
fn fold_developer_messages_into_instructions(
