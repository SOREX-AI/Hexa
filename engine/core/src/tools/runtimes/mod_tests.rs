...
fn runtime_path_prepends_records_runtime_path_prepend() {
    ...
    runtime_path_prepends.prepend(&mut env, PathBuf::from("/package/hexa-path").as_path());

    assert_eq!(
        env.get("PATH").map(String::as_str),
        Some("/package/hexa-path:/usr/bin:/bin"),
        "runtime PATH prepend should update the live exec environment"
    );
    assert_eq!(
        runtime_path_prepends.entries,
        vec!["/package/hexa-path"],
...
fn runtime_path_prepends_drops_empty_path_entries() {
    let mut env = HashMap::from([(
        "PATH".to_string(),
        ":/usr/bin:/package/hexa-path::/bin:".to_string(),
    )]);
    let mut runtime_path_prepends = RuntimePathPrepends::default();
...
fn apply_zsh_fork_path_prepend_uses_shell_parent() {
    ...
    apply_zsh_fork_path_prepend(
        ...
        PathBuf::from("/package/hexa-resources/zsh/bin/zsh").as_path(),
    );

    let expected = "/package/hexa-resources/zsh/bin:/usr/bin:/bin";
    assert_eq!(env.get("PATH").map(String::as_str), Some(expected));
    assert_eq!(
        runtime_path_prepends,
        RuntimePathPrepends {
            entries: vec!["/package/hexa-resources/zsh/bin".to_string()]
        }
    );
}
...
fn apply_zsh_fork_path_prepend_moves_existing_shell_parent_to_front() {
    let mut env = HashMap::from([(
        "PATH".to_string(),
        "/usr/bin:/package/hexa-resources/zsh/bin:/bin:/package/hexa-resources/zsh/bin"
            .to_string(),
    )]);
    let mut runtime_path_prepends = RuntimePathPrepends::default();
    ...
    assert_eq!(
        env.get("PATH").map(String::as_str),
        Some("/package/hexa-resources/zsh/bin:/usr/bin:/bin")
    );
    assert_eq!(
        runtime_path_prepends,
        RuntimePathPrepends {
...
fn run_snapshot_path_probe_with_runtime_path_prepend(
    ...
    let package_path_dir = dir.path().join("hexa-path");
