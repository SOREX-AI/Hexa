...
fn personal_access_token_exec_command(server: &MockServer, home: &TempDir) -> Command {
    let bin = codex_utils_cargo_bin::cargo_bin("hexa-engine").unwrap();
    ...
    cmd.arg("exec")
        ...
        .arg(repo_root())
        .arg("hello?");
    cmd.env("HEXA_ENGINE_HOME", home.path())
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    let bin2 = codex_utils_cargo_bin::cargo_bin("hexa-engine").unwrap();
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("HEXA_ENGINE_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy");
