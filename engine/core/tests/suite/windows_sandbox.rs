...
fn codex_home_for_windows_sandbox_test(name: &str) -> anyhow::Result<TestCodexHome> {
    if let Some(test_tmpdir) = std::env::var_os("TEST_TMPDIR") {
        ...
        // retries run in the same Windows VM, so keep HEXA_ENGINE_HOME stable within
        ...
        std::fs::create_dir_all(&codex_home).with_context(|| {
            format!(
                "create stable test HEXA_ENGINE_HOME {}",
                codex_home.display()
            )
        })?;
...
fn stage_windows_sandbox_helpers() -> anyhow::Result<()> {
    ...
    let resources_dir = test_exe_dir.join("hexa-resources");
    ...
    for helper_name in ["hexa-windows-sandbox-setup", "hexa-command-runner"] {
...
fn stage_windows_sandbox_cli(fixture_bin: &Path) -> anyhow::Result<(PathBuf, PathBuf)> {
    ...
    let resources_dir = fixture_bin.join("hexa-resources");
    std::fs::create_dir_all(&resources_dir)?;

    let codex_source = codex_utils_cargo_bin::cargo_bin("hexa-engine")?;
    ...
    let probe_source = codex_utils_cargo_bin::cargo_bin("hexa-windows-managed-deny-probe")?;
...
async fn windows_restricted_token_rejects_exact_and_glob_deny_read_policy() -> anyhow::Result<()> {
    ...
    let _codex_home_guard = EnvVarGuard::set("HEXA_ENGINE_HOME", codex_home.path().as_os_str());
...
async fn windows_elevated_enforces_deny_read_and_protects_setup_marker() -> anyhow::Result<()> {
    ...
    let bundled_skill_dir = user_profile.path().join(".hexashell/plugins/cache");
