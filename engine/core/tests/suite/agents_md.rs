...
use hexa_exec_server::LOCAL_ENVIRONMENT_ID;
use hexa_exec_server::REMOTE_ENVIRONMENT_ID;
...
-> Result<()> {
    ...
    let error = error
        .replace(&canonical_home_path, "$HEXA_ENGINE_HOME")
        .replace(&home_path, "$HEXA_ENGINE_HOME");
    insta::assert_snapshot!(error, @"
    Fatal error: Failed to initialize session: failed to load AGENTS.md instructions for environment `local`: failed to prepare fs sandbox: failed to prepare Seatbelt sandbox: writable root $HEXA_ENGINE_HOME/visualizations contains symlink component $HEXA_ENGINE_HOME/visualizations; symlinked writable roots are not supported.
    If this writable root is at or beneath HEXA_ENGINE_HOME and you trust its symlink targets, set `allow_symlinked_codex_home = true` at the top level of `$HEXA_ENGINE_HOME/config.toml` (normally `~/.hexashell/config.toml`) on the execution host, then restart Codex or its executor. This opt-out trusts targets outside HEXA_ENGINE_HOME and targets changed between commands. It does not apply to other writable roots.
    ");
