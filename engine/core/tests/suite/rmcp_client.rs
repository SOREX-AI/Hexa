...
async fn stdio_mcp_tool_call_includes_sandbox_state_meta(
    ...
    assert_eq!(
        ...
        SandboxState {
            ...
            hexa_linux_sandbox_exe: fixture.config.hexa_linux_sandbox_exe.clone(),
...
/// This test writes to a fallback credentials file in HEXA_ENGINE_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire HEXA_ENGINE_HOME through the code.
#[test]
#[serial(codex_home)]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 3: seed an isolated HEXA_ENGINE_HOME with fallback OAuth tokens for this
    // server so the test does not share credentials with other suite cases.
    let temp_home = Arc::new(tempdir()?);
    let _codex_home_guard = EnvVarGuard::set("HEXA_ENGINE_HOME", temp_home.path().as_os_str());
    ...
    let recovery_hint = if credential_config.is_local_environment() {
        format!("Run `hexa-engine mcp login {server_name}`.")
    } else {
