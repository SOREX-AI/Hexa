...
use hexa_exec_server::CreateDirectoryOptions;
use hexa_exec_server::ExecutorFileSystem;
use hexa_exec_server::RemoveOptions;
...
pub fn local(cwd: AbsolutePathBuf) -> TurnEnvironmentSelection {
    TurnEnvironmentSelection {
        environment_id: hexa_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
...
#[derive(Debug)]
pub struct TestEnv {
    environment: hexa_exec_server::Environment,
...
impl TestEnv {
    ...
    pub async fn local_with_exec_server_url(exec_server_url: Option<String>) -> Result<Self> {
        ...
        let selection = match exec_server_url {
            Some(_) => TurnEnvironmentSelection {
                environment_id: hexa_exec_server::REMOTE_ENVIRONMENT_ID.to_string(),
                cwd: PathUri::from_abs_path(&cwd),
                workspace_roots: vec![PathUri::from_abs_path(&cwd)],
                config: EnvironmentConfigState::FromThread,
            },
        ...
        let environment =
            hexa_exec_server::Environment::create_for_tests(exec_server_url.clone())?;
        Ok(Self {
            environment,
            exec_server_url,
            cwd,
    ...
    pub fn environment(&self) -> &hexa_exec_server::Environment {
        &self.environment
    }
...
pub async fn test_env() -> Result<TestEnv> {
    match test_environment() {
        remote_env @ (TestEnvironment::Docker { .. } | TestEnvironment::WineExec) => {
            ...
            let environment =
                hexa_exec_server::Environment::create_for_tests(Some(websocket_url.clone()))?;
            let cwd = remote_env
...
impl TestCodexBuilder {
    ...
    async fn build_with_home_and_base_url(
        ...
        #[cfg(target_os = "linux")]
        let hexa_linux_sandbox_exe = Some(
            crate::find_hexa_linux_sandbox_exe()
                .context("should find binary for hexa-linux-sandbox")?,
        );
        #[cfg(not(target_os = "linux"))]
        let hexa_linux_sandbox_exe = None;
        let local_runtime_paths = hexa_exec_server::ExecServerRuntimePaths::new(
            std::env::current_exe()?,
            hexa_linux_sandbox_exe,
        )?;
        let environment_manager = Arc::new(if include_local_environment {
            hexa_exec_server::EnvironmentManager::create_for_tests_with_local(
                exec_server_url,
                local_runtime_paths,
            )
            .await
        } else {
            hexa_exec_server::EnvironmentManager::create_for_tests(
                exec_server_url,
    ...
    async fn build_from_config(
        ...
        environment_manager: Arc<hexa_exec_server::EnvironmentManager>,
    ) -> anyhow::Result<TestCodex> {
        ...
        let code_mode_host_program = self
            ...
            .take()
            .or_else(|| codex_utils_cargo_bin::cargo_bin("hexa-code-mode-host").ok());
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = codex_utils_cargo_bin::cargo_bin("hexa-engine") {
            config.codex_self_exe = Some(path);
        } else if let Ok(path) = codex_utils_cargo_bin::cargo_bin("hexa-exec") {
            // `hexa-exec` also supports `--codex-run-as-apply-patch`, so use it
            ...
            && let Some(bin_dir) = exe.parent().and_then(|parent| parent.parent())
        {
            ...
            let codex_exec = bin_dir.join("hexa-exec");
            if codex.is_file() {
