...
/// avoids clobbering a developer’s real `~/.hexashell`.
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        hexa_linux_sandbox_exe: Some(
            find_hexa_linux_sandbox_exe().expect("should find binary for hexa-linux-sandbox"),
        ),
...
#[cfg(target_os = "linux")]
pub fn find_hexa_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    codex_utils_cargo_bin::cargo_bin("hexa-linux-sandbox")
}
...
#[macro_export]
macro_rules! hexa_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_hexa_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("hexa-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }
