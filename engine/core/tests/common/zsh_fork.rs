...
pub fn zsh_fork_runtime(test_name: &str) -> Result<Option<ZshForkRuntime>> {
    ...
    let Ok(main_execve_wrapper_exe) = codex_utils_cargo_bin::cargo_bin("hexa-execve-wrapper")
    else {
        eprintln!("skipping {test_name}: unable to resolve `hexa-execve-wrapper` binary");
        return Ok(None);
    };
