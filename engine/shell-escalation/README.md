# hexa-shell-escalation

This crate contains the Unix shell-escalation protocol implementation and the
`hexa-execve-wrapper` executable.

`hexa-execve-wrapper` receives the arguments to an intercepted `execve(2)` call and delegates the
...
- `Run`: `hexa-execve-wrapper` should invoke `execve(2)` on itself to run the original command
...
- `Escalate`: forward the file descriptors of the current process so the command can be run
  ...
  back to `hexa-execve-wrapper`.
- `Deny`: the server has declared the proposed command to be forbidden, so
  `hexa-execve-wrapper` prints an error to `stderr` and exits with `1`.
