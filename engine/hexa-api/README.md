...
Typed clients for Codex/OpenAI APIs built on top of the generic transport in `hexa-client`.
...
- Serves as the wire-level layer consumed by `hexa-core`; higher layers handle auth refresh and business logic.
...
All HTTP details (URLs, headers, retry/backoff policies, SSE framing) are encapsulated in `hexa-api` and `hexa-client`. Callers construct prompts/inputs using protocol types and work with typed streams of `ResponseEvent` or compacted `ResponseItem` values.
