...
Runtime orchestration for Phase 1 and Phase 2 still lives in `hexa-core` under
`codex-rs/core/src/memories/`.

## Crates

- `codex-rs/memories/read` (`hexa-memories-read`) owns the read path:
  memory developer-instruction injection, memory citation parsing, and
  read-usage telemetry classification.
- `codex-rs/memories/write` (`hexa-memories-write`) owns the write path:
...
- keeps the memories root itself as a git-baseline directory, initialized under
  `~/.hexashell/memories/.git` by `hexa-git-utils`
