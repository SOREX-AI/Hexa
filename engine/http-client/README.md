...
`hexa-http-client` is the low-level HTTP transport shared by Codex crates. It is the intended
...
Higher-level retry, SSE, and request-attempt telemetry policy remains in `hexa-client`.
...
If the existing wrapper surface cannot support a use case, extend `hexa-http-client` rather than
