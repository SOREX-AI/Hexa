//! Measures local HEXA_ENGINE_HOME storage once at standalone app-server startup.
//!
...
pub(crate) fn spawn(
    ...
    tokio::task::spawn_blocking(move || {
        let sizes = match directory_sizes(&codex_home, &shutdown) {
            Ok(sizes) => sizes,
            Err(error) => {
                tracing::debug!(error_kind = ?error.kind(), "Skipping HEXA_ENGINE_HOME size metrics");
                return;
            }
        };
