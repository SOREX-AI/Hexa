...
const SQLITE_RECOVERY_CONFIG_WARNING_SUMMARY: &str = "Hexa Engine rebuilt its local database.";
...
const OTEL_SERVICE_NAME: &str = "hexa-app-server";
#[cfg(debug_assertions)]
...
async fn init_sqlite_state_db_with_fresh_start_on_corruption(
    ...
    loop {
        ...
        emit_state_db_backup_warning(&format!(
            "Hexa Engine local database at {} appears damaged. Moving it into a backup folder so the app server can rebuild it from saved data.",
        ...
        for backup in &backups {
            emit_state_db_backup_warning(&format!(
                "Moved damaged Hexa Engine local database file {} to {}",
