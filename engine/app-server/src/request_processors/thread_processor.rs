...
const CODEX_TUI_CLIENT_NAME: &str = "hexa-tui";
...
impl ThreadRequestProcessor {
    ...
    fn build_thread_config_overrides(
        ...
        ConfigOverrides {
            ...
            hexa_linux_sandbox_exe: self.arg0_paths.hexa_linux_sandbox_exe.clone(),
    ...
    async fn read_stored_thread_for_resume(
        ...
        if stored_thread.archived_at.is_some() {
            ...
            return Err(invalid_request(format!(
                "session {thread_id} is archived. Run `hexa-engine unarchive {thread_id}` to unarchive it first."
            )));
        }
