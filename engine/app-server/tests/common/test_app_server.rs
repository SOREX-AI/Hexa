...
use hexa_exec_server::CODEX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR;
use hexa_exec_server::CODEX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR;
use hexa_exec_server::CODEX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR;
use hexa_exec_server::CODEX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR;
use hexa_exec_server::CODEX_EXEC_SERVER_URL_ENV_VAR;
...
pub struct TestAppServer {
    ...
    // removing an owned HEXA_ENGINE_HOME that may still be its cwd on Windows.
...
impl TestAppServer {
    /// Starts building a server with a temporary HEXA_ENGINE_HOME and the standard
    ...
    async fn new_with_program_env_and_args(
        ...
        let mut process = loop {
            ...
            if !process
                ...
                || retries == 2
            {
                break process.context("hexa-mcp-server proc should start")?;
            }
            retries += 1;
...
impl TestAppServerBuilder {
    /// Uses this existing HEXA_ENGINE_HOME instead of a temporary one.
    ...
    /// Builds a server with a temporary HEXA_ENGINE_HOME and automatic environment
    /// by default.
    pub async fn build(self) -> anyhow::Result<TestAppServer> {
        ...
        let mut program = match program {
            ...
            None => codex_utils_cargo_bin::cargo_bin("hexa-app-server")
        ...
        if !custom_program
            ...
            && let Ok(code_mode_host_program) =
                codex_utils_cargo_bin::cargo_bin("hexa-code-mode-host")
        {
