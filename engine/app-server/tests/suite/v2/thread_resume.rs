...
const CODEX_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT: &str = "You are Hexa Engine, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals.";
...
async fn goal_first_live_thread_appears_in_state_db_thread_list() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .without_managed_config()
        .with_env_overrides(&[("HEXA_SQLITE_HOME", Some(sqlite_home))])
        .build_initialized()
        .await?;
    ...
    assert!(
        message.contains(&format!("session {conversation_id} is archived"))
            && message.contains(&format!(
                "hexa-engine unarchive {conversation_id}` to unarchive it first"
            )),
