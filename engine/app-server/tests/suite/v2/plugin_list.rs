...
async fn plugin_list_omitted_cwds_excludes_server_project_config() -> Result<()> {
    ...
    std::fs::create_dir_all(codex_home.path().join(".hexa"))?;
  ...
  "plugins": [
    ...
    std::fs::write(
        codex_home.path().join(".hexa/config.toml"),
        format!(
...
async fn plugin_list_refreshes_plugins_from_each_cwd(
    ...
    for ((repo, name), version) in repos.iter().zip(names).zip(versions) {
        for directory in [".git", ".hexa", ".agents/plugins", "sample/.codex-plugin"] {
...
async fn plugin_catalogs_skip_invalid_project_config_and_report_cwd_error() -> Result<()> {
    ...
    for repo in [&invalid_repo, &valid_repo] {
        for directory in [".git", ".hexa", ".agents/plugins"] {
            std::fs::create_dir_all(repo.join(directory))?;
        }
        set_project_trust_level(codex_home.path(), repo, TrustLevel::Trusted)?;
    }
    std::fs::write(invalid_repo.join(".hexa/config.toml"), "invalid = [\n")?;
    std::fs::write(
        valid_repo.join(".hexa/config.toml"),
...
"#,
  ...
  "plugins": [
    ...
    std::fs::create_dir_all(workspace_enabled.path().join(".hexa"))?;
    std::fs::write(
        workspace_enabled.path().join(".hexa/config.toml"),
...
"#;
    ...
    if configured_in_later_cwd {
        ...
        std::fs::write(later_repo.path().join(".hexa/config.toml"), plugin_config)?;
...
async fn plugin_list_includes_remote_marketplaces_when_remote_plugin_enabled(
    ...
    let cwds = if project_configuration != ProjectPluginConfiguration::None {
        ...
        std::fs::write(repo.path().join(".hexa/config.toml"), config)?;
        std::fs::write(
...
"#,
    ...
    std::fs::write(
        repo.path().join(".hexa/config.toml"),
