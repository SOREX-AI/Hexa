...
`hexa-app-server` is the interface Codex uses to power rich interfaces such as the [Codex VS Code extension](https://marketplace.visualstudio.com/items?itemName=openai.chatgpt).
...
Similar to [MCP](https://modelcontextprotocol.io/), `hexa-app-server` supports bidirectional communication using JSON-RPC 2.0 messages (with the `"jsonrpc":"2.0"` header omitted on the wire).

Supported transports:

- stdio (`--stdio` or `--listen stdio://`, default): newline-delimited JSON (JSONL)
- websocket (`--listen ws://IP:PORT`): one JSON-RPC message per websocket text frame (**experimental / unsupported**)
- unix socket (`--listen unix://` or `--listen unix://PATH`): websocket connections over `$HEXA_ENGINE_HOME/app-server-control/app-server-control.sock` or a custom socket path, using the standard HTTP Upgrade handshake (also supported on Windows)
...
The unix socket transport is intended for local app-server control-plane clients. `hexa-app-server proxy`
opens exactly one raw stream connection to `$HEXA_ENGINE_HOME/app-server-control/app-server-control.sock`
...
Currently, you can dump a TypeScript version of the schema using `hexa-app-server generate-ts`, or a JSON Schema bundle via `hexa-app-server generate-json-schema`. Each output is specific to the version of Codex you used to run the command, so the generated artifacts are guaranteed to match that version.

```
hexa-app-server generate-ts --out DIR
hexa-app-server generate-json-schema --out DIR
```
...
Applications building on top of `hexa-app-server` should identify themselves via the `clientInfo` parameter.
...
- `memory/reset` — experimental; clear the current `HEXA_ENGINE_HOME/memories` directory and reset persisted memory stage data in sqlite while preserving existing thread memory modes; returns `{}` on success.
...
{ "method": "turn/start", "id": 33, "params": {
    ...
    "input": [
        ...
        { "type": "skill", "name": "skill-creator", "path": "/Users/me/.hexashell/skills/skill-creator/SKILL.md" }
    ]
} }
{ "id": 33, "result": { "turn": {
    "id": "turn_457",
  ...
  "params": {
    ...
    "input": [
      ...
      },
      {
        "type": "skill",
        "name": "skill-creator",
        "path": "/Users/me/.hexashell/skills/skill-creator/SKILL.md"
      }
    ]
  }
}
...
```json
{
  ...
  "id": 27,
  "params": {
    "path": "/Users/alice/.hexashell/skills/skill-creator/SKILL.md",
    "name": null,
    "enabled": false
  }
}
...
For linked Git worktrees, project hook declarations come from the matching `.hexa/` folders in the root checkout rather than from divergent hook declarations stored only in the linked worktree. This keeps each repo on one authoritative project-hook definition and one trust state.
...
```json
{
  ...
  "result": {
    "data": [{
      "cwd": "/Users/me/project",
      "hooks": [{
        "key": "/Users/me/.hexashell/config.toml:pre_tool_use:0:0",
        ...
        "sourcePath": "/Users/me/.hexashell/config.toml",
        "source": "user",
        "pluginId": null,
        "displayOrder": 0,
        "enabled": true,
...
```json
{
  ...
  "params": {
    "edits": [{
      "keyPath": "hooks.state",
      "value": {
        "/Users/me/.hexashell/config.toml:pre_tool_use:0:0": {
          "enabled": false
        }
      },
...
- **Personal access token (`personalAccessToken`)**: Codex uses a ChatGPT-backed personal access token loaded outside the app-server login RPCs, such as with `hexa-engine login --with-access-token` or `CODEX_ACCESS_TOKEN`.
...
active user config. Neither flow changes `$HEXA_ENGINE_HOME/.env`. Existing loaded sessions keep their
...
profile or environment credentials leaves existing keys in `$HEXA_ENGINE_HOME/.env` unchanged.
...
any Codex-managed credentials, and leaves AWS-managed credentials and `$HEXA_ENGINE_HOME/.env` unchanged.
...
`$HEXA_ENGINE_HOME/.env` are left untouched.
...
`hexa-app-server` schema generation defaults to the stable API surface (experimental fields and methods filtered out). Pass `--experimental` to include experimental methods/fields in generated TypeScript or JSON schema:
...
hexa-app-server generate-ts --out DIR --experimental
hexa-app-server generate-json-schema --out DIR --experimental
```
...
5. Verify the protocol crate:
   ...
   ```bash
   just test -p hexa-app-server-protocol
   ```
