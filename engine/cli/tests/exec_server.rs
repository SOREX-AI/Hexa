...
use hexa_exec_server::EnvironmentInfo;
use hexa_exec_server::ExecParams;
use hexa_exec_server::ExecServerClient;
use hexa_exec_server::NoiseChannelIdentity;
use hexa_exec_server::NoiseChannelPublicKey;
use hexa_exec_server::NoiseRendezvousConnectArgs;
use hexa_exec_server::NoiseRendezvousConnectBundle;
use hexa_exec_server::ProcessId;
...
fn codex_command(codex_home: &Path) -> Result<assert_cmd::Command> {
    let mut cmd = assert_cmd::Command::new(codex_utils_cargo_bin::cargo_bin("hexa-engine")?);
    cmd.env("HEXA_ENGINE_HOME", codex_home);
    Ok(cmd)
}
...
fn local_exec_server_allows_disabled_parent_lifetime_environment_variable() -> Result<()> {
    ...
    let mut cmd = codex_command(codex_home.path())?;
    cmd.env(
        hexa_exec_server::CODEX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE_ENV_VAR,
        "false",
    )
...
"#
    ...
    std::fs::copy(
        codex_utils_cargo_bin::cargo_bin("hexa-engine")?,
        &executable,
    )?;
    let manifest = package.path().join("hexa-package.json");
    std::fs::write(&manifest, r#"{"version":"1.2.3-alpha.4"}"#)?;

    let mut command = tokio::process::Command::new(executable);
    command
        .env("HEXA_ENGINE_HOME", codex_home.path())
    ...
    client
        .exec(ExecParams {
            ...
            shell_snapshot: None,
            env_policy: Some(hexa_exec_server::ExecEnvPolicy {
...
fn local_exec_server_exits_successfully_on_sigterm() -> Result<()> {
    let codex_home = TempDir::new()?;
    let mut child = std::process::Command::new(codex_utils_cargo_bin::cargo_bin("hexa-engine")?)
        .env("HEXA_ENGINE_HOME", codex_home.path())
