...
fn queue_rejects_empty_message() -> Result<()> {
    let output = std::process::Command::new(codex_utils_cargo_bin::cargo_bin("hexa-engine")?)
...
async fn run_remote_queue_command(response: QueueResponse) -> Result<(Output, Value)> {
    ...
    let output = tokio::process::Command::new(codex_utils_cargo_bin::cargo_bin("hexa-engine")?)
