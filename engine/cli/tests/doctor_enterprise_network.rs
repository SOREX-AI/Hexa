...
"#,
        ...
        std::fs::create_dir(workspace.path().join(".hexa"))?;
        std::fs::write(
        ...
        let output = Command::new(codex_utils_cargo_bin::cargo_bin("hexa-engine")?)
...
async fn invalid_custom_ca_falls_back_to_system_roots() -> Result<()> {
    ...
    for sandbox in [None, Some("seatbelt")] {
        let mut command = Command::new(codex_utils_cargo_bin::cargo_bin("hexa-engine")?);
        command
            .args(["doctor", "--json"])
            .env("HEXA_ENGINE_HOME", codex_home.path())
