...
async fn cli_main(
    ...
    match subcommand {
        None | Some(Subcommand::Agents(_)) => {
            ...
            if open_agents_overview {
                if interactive.prompt.is_some() || !interactive.images.is_empty() {
                    anyhow::bail!("`hexa-engine agents` does not accept an initial prompt or images");
                }
        ...
        Some(Subcommand::Cloud(mut cloud_cli)) => {
            ...
            codex_cloud_tasks::run_main(cloud_cli, arg0_paths.hexa_linux_sandbox_exe.clone())
                .await?;
        }
        Some(Subcommand::Sandbox(mut sandbox_cli)) => {
            ...
            codex_cli::run_command_under_seatbelt(
                sandbox_cli,
                arg0_paths.hexa_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            .await?;
            #[cfg(target_os = "linux")]
        ...
        Some(Subcommand::ResponsesApiProxy(args)) => {
            ...
            tokio::task::spawn_blocking(move || hexa_responses_api_proxy::run_main(args))
                .await??;
        }
...
async fn run_exec_server_command(
    ...
    let runtime_paths = hexa_exec_server::ExecServerRuntimePaths::new(
        codex_self_exe,
        arg0_paths.hexa_linux_sandbox_exe.clone(),
    )?;
    if let Some(base_url) = cmd.remote {
        ...
        let auth_provider =
            load_exec_server_remote_auth_provider(&config, &base_url, cmd.use_agent_identity_auth)
                .await?;
        let mut remote_config = hexa_exec_server::RemoteEnvironmentConfig::new(
            base_url,
            environment_id,
            auth_provider,
...
async fn run_debug_prompt_input_command(
    ...
    let overrides = ConfigOverrides {
        ...
        hexa_linux_sandbox_exe: arg0_paths.hexa_linux_sandbox_exe,
...
mod tests {
    ...
    fn format_exit_messages_includes_resume_hint_for_fatal_exit() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run hexa-engine resume 123e4567-e89b-12d3-a456-426614174000"
                    .to_string(),
            ]
        );
    }
    ...
    fn format_exit_messages_names_picker_item_when_thread_has_name() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2".to_string(),
                "To continue this session, run hexa-engine resume, then select my-thread (123e4567-e89b-12d3-a456-426614174000)".to_string(),
            ]
        );
    }
