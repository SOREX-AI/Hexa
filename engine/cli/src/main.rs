...
use hexa_exec::Cli as ExecCli;
use hexa_exec::Command as ExecCommand;
use hexa_exec::ReviewArgs;
use hexa_exec_server::ExecServerRuntimePaths;
use hexa_execpolicy::ExecPolicyCheckCommand;
use hexa_responses_api_proxy::Args as ResponsesApiProxyArgs;
use hexa_tui::AppExitInfo;
use hexa_tui::Cli as TuiCli;
use hexa_tui::ExitReason;
...
#[clap(
    ...
    // Hexa stages this upstream-compatible binary under a product-specific
...
enum Subcommand {
    ...
    /// Run Hexa Engine non-interactively.
    ...
    /// Manage external MCP servers for Hexa Engine.
    Mcp(McpCli),

    /// Manage Hexa Engine plugins.
    Plugin(PluginCli),

    /// Start Hexa Engine as an MCP server (stdio).
    ...
    /// Show Hexa Engine update guidance.
    Update,

    /// Diagnose local Hexa Engine config, auth, and runtime health.
    Doctor(DoctorCommand),

    /// Run commands within a Hexa-provided sandbox.
    ...
    /// Apply the latest diff produced by Hexa Engine as a `git apply` to your local working tree.
...
struct ReviewCommand {
    /// Error out when config.toml contains fields that are not recognized by this version of Hexa Engine.
...
struct UnsupportedSandboxArgs {
    /// Layer $HEXA_ENGINE_HOME/<name>.config.toml on top of the base user config.
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv OPENAI_API_KEY | hexa-engine login --with-api-key`)"
    )]
    with_api_key: bool,

    #[arg(
        long = "with-access-token",
        help = "Read the access token from stdin (e.g. `printenv CODEX_ACCESS_TOKEN | hexa-engine login --with-access-token`)"
    )]
...
struct ExecServerCommand {
    ...
    request_dispatch_mode: hexa_exec_server::RequestDispatchMode,
    ...
    #[arg(
        ...
        env = hexa_exec_server::CODEX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE_ENV_VAR,
...
enum AppServerSubcommand {
    ...
    /// [internal] Generate internal JSON Schema artifacts for Hexa Engine tooling.
    #[clap(hide = true)]
...
fn handle_app_exit(exit_info: AppExitInfo) -> anyhow::Result<()> {
    ...
    if update_action.is_some() {
        eprintln!(
            "Hexa Engine updates are installed through Hexa: `npm run hexa:upstream -- <ref> --apply`."
        );
    }
    Ok(())
}
...
fn run_update_command() -> anyhow::Result<()> {
    anyhow::bail!(
        "Hexa Engine updates are managed by Hexa. Use `npm run hexa:upstream -- <ref> --apply`."
    )
}
...
async fn run_session_archive_cli_command(
    action: hexa_tui::SessionArchiveAction,
    ...
    hexa_tui::run_session_archive_command(
        action,
        target,
        hexa_tui::SessionArchiveCommandOptions {
            cli: interactive,
            arg0_paths,
...
fn delete_action(target: &str, force: bool) -> anyhow::Result<hexa_tui::SessionArchiveAction> {
    ...
    let confirmation = match force {
        true => hexa_tui::DeleteConfirmation::Skip,
        false => hexa_tui::DeleteConfirmation::Prompt,
    };
    Ok(hexa_tui::SessionArchiveAction::Delete(confirmation))
}
...
async fn cli_main(
    ...
    {
        anyhow::bail!("`hexa-engine agents` received conflicting remote server endpoints");
    }
    ...
    match subcommand {
        None | Some(Subcommand::Agents(_)) => {
            ...
            if open_agents_overview {
                if interactive.prompt.is_some() || !interactive.images.is_empty() {
                    anyhow::bail!(
                        "`hexa-engine agents` does not accept an initial prompt or images"
                    );
                }
                ...
                {
                    anyhow::bail!(
                        "`hexa-engine agents` cannot apply local provider or additional-directory overrides to a remote server"
                    );
                }
                if is_workload_identity_selected() {
                    anyhow::bail!(
                        "`hexa-engine agents` is unavailable while workload identity is active"
                    );
                }
                if root_remote.is_none() {
                    ...
                    anyhow::bail!("`hexa-engine agents` requires `--remote` on this platform");
                }
        ...
        Some(Subcommand::Exec(mut exec_cli)) => {
            ...
            hexa_exec::run_main(exec_cli, arg0_paths.clone()).await?;
        }
        ...
        Some(Subcommand::McpServer(McpServerCommand { strict_config })) => {
            eprintln!(
                "warning: `hexa-engine mcp-server` is deprecated and will be removed in a future release."
            );
            reject_remote_mode_for_subcommand(
                root_remote.as_deref(),
                root_remote_auth_token_env.as_deref(),
        ...
        Some(Subcommand::AppServer(app_server_cli)) => {
            ...
            match subcommand {
                ...
                Some(AppServerSubcommand::Proxy(proxy_cli)) => {
                    ...
                    hexa_stdio_to_uds::run(socket_path.as_path()).await?;
                }
        ...
        Some(Subcommand::Archive(cmd)) => {
            let output = run_session_archive_cli_command(
                hexa_tui::SessionArchiveAction::Archive,
                cmd,
                interactive,
                root_config_overrides.clone(),
                root_remote.clone(),
        ...
        Some(Subcommand::Unarchive(cmd)) => {
            let output = run_session_archive_cli_command(
                hexa_tui::SessionArchiveAction::Unarchive,
                cmd,
                interactive,
                root_config_overrides.clone(),
                root_remote.clone(),
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.with_api_key && login_cli.with_access_token {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv OPENAI_API_KEY | hexa-engine login --with-api-key`."
                        );
                        std::process::exit(1);
        ...
        Some(Subcommand::Sandbox(mut sandbox_cli)) => {
            ...
            #[cfg(not(any(target_os = "macos", target_os = "linux", target_os = "windows")))]
            {
                ...
                anyhow::bail!("`hexa-engine sandbox` is not supported on this operating system");
            }
        }
        ...
        Some(Subcommand::ResponsesApiProxy(args)) => {
            ...
            tokio::task::spawn_blocking(move || hexa_responses_api_proxy::run_main(args)).await??;
        }
...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        _ => anyhow::bail!(
            "--profile only applies to runtime commands and `hexa-engine mcp`: `hexa-engine`, `codex exec`, `codex review`, `hexa-engine resume`, `hexa-engine queue`, `hexa-engine archive`, `hexa-engine delete`, `hexa-engine unarchive`, `hexa-engine fork`, `hexa-engine mcp`, `hexa-engine sandbox`, and `codex debug prompt-input`."
        ),
    }
}
...
async fn run_exec_server_command(
    ...
    let codex_self_exe = arg0_paths
        .codex_self_exe
        .clone()
        .ok_or_else(|| anyhow::anyhow!("Hexa Engine executable path is not configured"))?;
    let runtime_paths =
    ...
    if let Some(base_url) = cmd.remote.take() {
        ...
        let mut remote_config = hexa_exec_server::RemoteEnvironmentConfig::new_with_transport(
            base_url,
            environment_id,
            if direct_transport {
                hexa_exec_server::RemoteEnvironmentTransport::Direct
            } else {
                hexa_exec_server::RemoteEnvironmentTransport::Noise
            },
            auth_provider,
            ...
            async move {
                ...
                match cmd.command {
                    Some(ExecServerSubcommand::Forward { connect }) => {
                        hexa_exec_server::run_remote_environment_forward_until_shutdown(
                            remote_config,
                            connect,
                            shutdown,
                        )
                    ...
                    None => {
                        hexa_exec_server::run_remote_environment_until_shutdown(
                            remote_config,
                            runtime_paths,
                            shutdown,
                        )
        ...
        let listen_url = cmd
            .listen
            .unwrap_or_else(|| hexa_exec_server::DEFAULT_LISTEN_URL.to_string());
        let run = exec_server_telemetry::run_until_shutdown(
            hexa_exec_server::run_main_with_telemetry(
                &listen_url,
                runtime_paths,
                telemetry,
                http_client_factory,
...
async fn load_exec_server_remote_auth_provider(
    ...
    let (auth_manager, auth) = load_exec_server_remote_auth(
        config,
        "remote exec-server registration requires ChatGPT authentication or API key authentication; run `hexa-engine login` or set CODEX_API_KEY",
    )
    .await?;
...
async fn run_interactive_tui(
    ...
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". Hexa's interactive TUI may not work in this terminal."
        );
        ...
        match local_state_db::backup_files_for_fresh_start(startup_error).await {
            ...
            Err(backup_err) => {
                ...
                return Ok(AppExitInfo::fatal(format!(
                    "failed to move damaged Hexa Engine local database files into a backup folder automatically: {backup_err}"
                )));
            }
        }
    }
...
fn resolve_remote_endpoint(
    remote: Option<String>,
    remote_auth_token_env: Option<String>,
) -> std::io::Result<Option<hexa_tui::RemoteAppServerEndpoint>> {
    let mut remote_endpoint = remote
        .as_deref()
        .map(hexa_tui::resolve_remote_addr)
        .transpose()
        .map_err(std::io::Error::other)?;
    if let Some(remote_auth_token_env) = remote_auth_token_env {
        ...
        if !hexa_tui::remote_addr_supports_auth_token(endpoint) {
            return Err(std::io::Error::other(
        ...
        let auth_token = read_remote_auth_token_from_env_var(&remote_auth_token_env)
            .map_err(std::io::Error::other)?;
        let hexa_tui::RemoteAppServerEndpoint::WebSocket {
            auth_token: slot, ..
        } = endpoint
        else {
            return Err(std::io::Error::other(
...
/// Build the final `TuiCli` for a `hexa-engine resume` invocation.
fn finalize_resume_interactive(
    ...
    // configuration surface area as `hexa-engine` without additional flags.
...
/// Build the final `TuiCli` for a `hexa-engine fork` invocation.
...
mod tests {
    ...
    fn exec_resume_last_accepts_prompt_positional() {
        ...
        let Some(hexa_exec::Command::Resume(args)) = exec.command else {
    ...
    fn sample_exit_info(conversation_id: Option<&str>, thread_name: Option<&str>) -> AppExitInfo {
        ...
        AppExitInfo {
            token_usage,
            thread_id,
            resume_hint: thread_id.map(|thread_id| hexa_tui::ResumableThread {
                thread_id,
    ...
    async fn format_exit_messages_preserves_auth_env_through_tui_runner() {
        let exit_info = run_tui_with_recovery(
            || async {
                ...
                exit_info.disconnect_info = Some(hexa_tui::DisconnectInfo {
                    command: vec![
                        "codex".to_string(),
                        "--remote".to_string(),
    ...
    fn format_exit_messages_includes_resume_hint_for_fatal_exit() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "  hexa-engine resume 123e4567-e89b-12d3-a456-426614174000".to_string(),
            ]
        );
    }
        ...
        insta::allow_duplicates! {
            for thread_name in [None, Some("")] {
                ...
                To continue this session, run:
                  hexa-engine resume 123e4567-e89b-12d3-a456-426614174000
                ");
            }
        }
    }
    ...
    fn format_exit_messages_applies_color_when_enabled() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2",
                "To continue this session, run:",
                "  \u{1b}[36mhexa-engine resume 123e4567-e89b-12d3-a456-426614174000\u{1b}[39m",
            ]
        );
    }
    ...
    fn format_exit_messages_names_picker_item_when_thread_has_name() {
        ...
        To continue this session, run:
          hexa-engine resume 123e4567-e89b-12d3-a456-426614174000
        Or run hexa-engine resume and select my-thread.
        ");
    }
    ...
    fn format_exit_messages_colors_commands_and_thread_name() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run:",
                "  \u{1b}[36mhexa-engine resume 123e4567-e89b-12d3-a456-426614174000\u{1b}[39m",
                "Or run \u{1b}[36mhexa-engine resume\u{1b}[39m and select \u{1b}[36mmy-thread\u{1b}[39m.",
            ]
        );
    }
    ...
    fn root_strict_config_is_rejected_for_unsupported_subcommands() {
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `hexa-engine mcp`"
        );
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `hexa-engine remote-control`"
        );
    }
