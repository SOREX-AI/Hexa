//! Implements the `hexa-engine doctor` diagnostic report.
//!
...
use codex_install_context::HexaPackageLayout;
...
///
/// This is the CLI entry point for hexa-engine doctor. It does not repair issues;
            ...
            let (
                ...
                async {
                    run_sync_check("auth", progress.clone(), || match &auth_manager_result {
                        ...
                        .remediation(
                            "Fix the reported authentication error, then rerun hexa-engine doctor.",
                        ),
                    })
                },
            ...
            let (
                ...
                async {
                    run_sync_check("config", progress.clone(), || {
                        ...
                        .remediation("Fix the reported config error, then rerun hexa-engine doctor.")
                    })
                },
                async {
...
fn config_overrides_from_interactive(
    ...
    ConfigOverrides {
        ...
        hexa_linux_sandbox_exe: arg0_paths.hexa_linux_sandbox_exe.clone(),
...
/// JSON support report emitted by `hexa-engine doctor --json`.
///
...
fn config_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    details.push(format!("HEXA_ENGINE_HOME: {}", config.codex_home.display()));
    ...
    match load_auth_dot_json(
        ...
        Ok(Some(auth)) => {
            ...
            if status == CheckStatus::Fail {
                check =
                    check.remediation("Run hexa-engine login again or provide a supported auth env var.");
            }
            check
        }
        ...
        .details(details)
        .remediation("Run hexa-engine login or provide an API key through a supported auth env var."),
        ...
        .detail(err.to_string())
        .remediation("Fix auth storage access or run hexa-engine login again."),
    }
}
...
async fn state_check(config: &Config, command: &DoctorCommand) -> DoctorCheck {
    let mut details = Vec::new();
    path_readiness(&mut details, "HEXA_ENGINE_HOME", &config.codex_home);
...
fn fallback_state_check() -> DoctorCheck {
    ...
    match codex_home {
        Ok(path) => DoctorCheck::new(
            ...
            "state",
            CheckStatus::Ok,
            "HEXA_ENGINE_HOME was resolved without config",
        )
        .detail(format!("HEXA_ENGINE_HOME: {}", path.display())),
        Err(err) => DoctorCheck::new(
            "state.paths",
            "state",
            CheckStatus::Warning,
            "HEXA_ENGINE_HOME could not be resolved",
        )
        .detail(err.to_string()),
    }
}
...
mod tests {
    ...
    fn config_overrides_from_interactive_preserves_global_options() {
        ...
        let arg0_paths = Arg0DispatchPaths {
            ...
            hexa_linux_sandbox_exe: Some(PathBuf::from("/bin/hexa-linux-sandbox")),
            main_execve_wrapper_exe: Some(PathBuf::from("/bin/hexa-execve-wrapper")),
        };
        ...
        assert_eq!(
            overrides.hexa_linux_sandbox_exe,
            arg0_paths.hexa_linux_sandbox_exe
        );
        assert_eq!(
