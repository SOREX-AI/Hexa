            ...
            let (
                ...
                async {
                    run_sync_check("config", progress.clone(), || {
                        ...
                        .remediation(
                            "Fix the reported config error, then rerun hexa-engine doctor.",
                        )
                    })
                },
                async {
...
fn auth_check(config: &Config) -> DoctorCheck {
    ...
    match load_auth_dot_json(
        ...
        Ok(Some(auth)) => {
            ...
            if status == CheckStatus::Fail {
                check = check.remediation(
                    "Run hexa-engine login again or provide a supported auth env var.",
                );
            }
            check
        }
        ...
        .details(details)
        .remediation(
            "Run hexa-engine login or provide an API key through a supported auth env var.",
        ),
...
mod tests {
    ...
    fn config_overrides_from_interactive_preserves_global_options() {
        ...
        let arg0_paths = Arg0DispatchPaths {
            ...
            codex_linux_sandbox_exe: Some(PathBuf::from("/bin/hexa-linux-sandbox")),
