...
use hexa_exec_server::HttpClient;
use hexa_exec_server::RouteAwareHttpClient;
...
/// - `add`    — add a server launcher entry to `~/.hexashell/config.toml`
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "hexa-engine mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let codex_home = find_codex_home().context("failed to resolve HEXA_ENGINE_HOME")?;
    ...
    match login_support {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `hexa-engine mcp login {name}` to login."
        ),
    }
...
async fn run_list(config: &Config, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `hexa-engine mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
async fn run_get(config: &Config, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: hexa-engine mcp remove {}", get_args.name);
