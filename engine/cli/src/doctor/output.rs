...
fn write_footer(out: &mut String, options: HumanOutputOptions) {
    if options.show_details {
        ...
        let _ = writeln!(
            ...
            "{}",
            dim(
                "Run hexa-engine doctor without --summary for detailed diagnostics.",
                options
            )
        );
        let _ = writeln!(
...
mod tests {
    ...
    fn sample_report() -> DoctorReport {
        let checks = vec![
            ...
            .remediation("Run `hexa-engine login`."),
            DoctorCheck::new(
                "updates.status",
                "updates",
                CheckStatus::Ok,
...
Notes
   ...
   ✗ auth         token expired - Run `hexa-engine login`.
─────────────────────────────────────────────────────────────
...
",
    ...
    fn render_human_report_snapshot_covers_environment_rows() {
        ...
        *targets = "exclusion targets: verified Hexa app and required helpers".into();
        ...
        report.checks.extend([
            ...
            .detail("running: true")
            .detail("log directory: $HOME/Library/Logs/com.hexa.engine"),
            DoctorCheck::new(
...
Notes
   [!!] terminal     narrow terminal
   [XX] auth         token expired - Run `hexa-engine login`.
-------------------------------------------------------------
