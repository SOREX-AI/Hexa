...
#[cfg(target_os = "windows")]
const WINDOWS_SETUP_REMEDIATION: &str = "run hexa-engine sandbox setup --elevated --user <end-user> --codex-home <authoritative-home> from an elevated shell";
...
pub(super) fn sandbox_check(config: &Config, arg0_paths: &Arg0DispatchPaths) -> DoctorCheck {
    ...
    push_path_detail(
        &mut details,
        "hexa-linux-sandbox helper",
        arg0_paths.hexa_linux_sandbox_exe.as_deref(),
    );
    push_path_detail(
        &mut details,
    ...
    if let Some(helper) = arg0_paths.hexa_linux_sandbox_exe.as_deref()
        && !helper.exists()
    {
                ...
                if let Some(report) = report {
                    let remediation = match report.code {
                        ...
                        | SetupErrorCode::HelperReadAclHelperSpawnFailed => {
                            "repair the installed Hexa helpers or ask IT to allow their execution"
                        }
                        ...
                        | SetupErrorCode::HelperFirewallRuleVerifyFailed => {
                            "ask IT to allow Hexa sandbox rules in managed Windows Firewall policy"
                        }
                        SetupErrorCode::OrchestratorPayloadSerializeFailed
                        | SetupErrorCode::HelperRequestArgsFailed
                        | SetupErrorCode::HelperUnknownError => {
                            "repair or reinstall the Hexa Engine from an approved distribution"
                        }
                    };
                    check
                        .details
