...
fn disk_capacity_is_reported_without_loaded_configuration() {
    ...
    for label in ["HEXA_ENGINE_HOME available: ", "worktree available: "] {
        assert!(
            check
                .details
                .iter()
...
fn disk_capacity_thresholds_produce_complete_diagnostics() {
    for (available, status, measured, summary) in [
        ...
        let issues = ["HEXA_ENGINE_HOME", "worktree"]
            .into_iter()
            .filter(|_| status != "ok")
            .map(|label| {
                json!({
        ...
        assert_eq!(
            ...
            json!({
                ...
                "details": [
                    ...
                    format!("HEXA_ENGINE_HOME available: {measured}"),
    ...
    assert_eq!(
        ...
        json!({
            ...
            "details": [
                "warning threshold: 5.0 GiB",
                "failure threshold: 1.0 GiB",
                "HEXA_ENGINE_HOME available: unavailable (PermissionDenied)",
