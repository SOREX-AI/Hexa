...
pub(super) fn endpoint_check(inspection: EndpointInspection) -> DoctorCheck {
    ...
    let targets = if cfg!(target_os = "windows") {
        "signed Hexa app; codex.exe; HexaSandboxSetup.exe; HexaCommandRunner.exe; HexaCodeModeHost.exe"
    } else if cfg!(target_os = "macos") {
        "signing team 2DC432GLL2 plus the installed Hexa app identity, signed codex agent, and required helpers"
    } else {
        "verified Hexa app, codex agent, and required helpers"
    };
    let mut check = DoctorCheck::new(
        "security.endpoint",
        "security",
        CheckStatus::Warning,
        "endpoint protection detected; Hexa exclusions are unverified",
    )
    .detail(format!("endpoint products: {}", products.join(", ")))
    .detail(format!("exclusion targets: {targets}"))
    .detail("Hexa exclusions: not verified")
    .remediation(
        "ask your security administrator to verify Hexa exclusions and required helper allowances",
    );
    ...
    for product in products {
        let remedy = match product {
            "CrowdStrike Falcon" => {
                "CrowdStrike Falcon: Add a certificate or IOA exclusion for Hexa. If sensor overhead continues, exclude the Hexa Engine agent from sensor visibility. Keep monitoring descendant processes."
            }
            "BeyondTrust Privilege Management" => {
                "BeyondTrust: Remove Codex from application blocking rules. Add allow rules for Hexa helper executables. Do not grant administrator privileges."
            }
            "Microsoft Defender" => {
                "Microsoft Defender: Add a certificate or executable-path exclusion for Hexa and its helpers. If Attack Surface Reduction blocks Codex, add a rule exclusion. If Controlled Folder Access blocks Codex, allow the app."
            }
            "SentinelOne" => {
                "SentinelOne: Add a signer, file-hash, or executable-path exclusion for Hexa and its helpers."
            }
            "Jamf Protect" => {
                "Jamf Protect: Add an Override Threat Prevention exception for Hexa app and helper signing identities. If analytics cause delays, add an Ignore System Events for Analytics exception."
            }
            _ => {
                "Add an exclusion for Hexa and its helpers. Use the endpoint product instructions."
            }
        };
        check = check.issue(
            DoctorIssue::new(
                CheckStatus::Warning,
                format!("{product} can interfere with Codex. Verify Hexa exclusions."),
            )
            .measured("not verified")
            .expected("Hexa application and helper exclusions")
            .remedy(remedy)
            .field("Hexa exclusions"),
        );
    }
