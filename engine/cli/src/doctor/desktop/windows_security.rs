...
fn event_target(value: &str) -> Option<&'static str> {
    ...
    match name {
        "HexaSandboxSetup.exe" => Some("sandbox_setup"),
        "HexaCommandRunner.exe" => Some("command_runner"),
...
fn classify(channels: &[Option<Vec<Evidence>>]) -> DoctorCheck {
    ...
    if status != CheckStatus::Ok {
        check = check.remediation(
            "ask your organization's security administrator to review endpoint security events and the approved Hexa application policy",
        );
    }
    check
}
