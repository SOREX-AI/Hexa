...
pub(crate) struct SandboxSetupCommand {
    ...
    /// Windows user that will run Hexa after managed deployment.
    #[arg(
        long = "user",
        value_name = "USER",
    ...
    /// Use the current Windows user as the Hexa user.
    #[arg(
        long = "current-user",
    ...
    /// HEXA_ENGINE_HOME for the Hexa user. Required with --user.
...
impl SandboxSetupCommand {
    fn setup_level(&self) -> anyhow::Result<SandboxSetupLevel> {
        if self.elevated_sandbox_level {
            ...
            anyhow::bail!("`hexa-engine sandbox setup` currently requires --elevated");
        }
    }
}
...
async fn run_elevated(
    ...
    let config = ConfigBuilder::default()
        ...
        .build()
        .await
        .context("failed to load target user's Hexa config for sandbox provisioning")?;
