...
const CLIENT_NAME: &str = "hexa_app_server_daemon";
    ...
    let initialize = JSONRPCMessage::Request(JSONRPCRequest {
        ...
        params: Some(serde_json::to_value(InitializeParams {
            client_info: ClientInfo {
                ...
                title: Some("Hexa App Server Daemon".to_string()),
...
mod tests {
    ...
    fn parses_version_from_codex_user_agent() {
        assert_eq!(
            parse_version_from_user_agent(
                "hexa_app_server_daemon/1.2.3 (Linux 6.8.0; x86_64) codex_cli_rs/1.2.3",
            )
            .expect("version"),
            "1.2.3"
        );
    ...
    fn rejects_user_agent_without_version() {
        assert!(parse_version_from_user_agent("hexa_app_server_daemon").is_err());
    }
}
