...
const STATE_DIR_NAME: &str = "hexa-app-server-daemon";
...
fn ensure_supported_platform() -> Result<()> {
    Err(anyhow!(
        "Hexa app-server daemon lifecycle is only supported on Unix platforms"
    ))
}
...
impl Daemon {
    ...
    async fn restart(&self) -> Result<LifecycleOutput> {
        ...
        {
            return Err(anyhow!(
                "app server is running but is not managed by Hexa app-server daemon"
            ));
        }
    ...
    fn ensure_managed_codex_bin(&self) -> Result<()> {
        ...
        let managed_engine_path = self.managed_codex_bin.display();
        Err(anyhow!(
