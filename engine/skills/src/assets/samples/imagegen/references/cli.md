...
Set a stable path to the skill CLI (default `HEXA_ENGINE_HOME` is `~/.hexashell`):

```
export HEXA_ENGINE_HOME="${HEXA_ENGINE_HOME:-$HOME/.codex}"
export IMAGE_GEN="$HEXA_ENGINE_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
