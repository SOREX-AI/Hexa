...
Ask clarifying questions only when the missing information matters and cannot be reasonably inferred. Respect a user-specified location; otherwise create discoverable skills in `$HEXA_ENGINE_HOME/skills`, or `~/.hexashell/skills` when `HEXA_ENGINE_HOME` is unset.
...
```bash
scripts/init_skill.py my-skill --path "${HEXA_ENGINE_HOME:-$HOME/.codex}/skills"
scripts/init_skill.py my-skill --path "${HEXA_ENGINE_HOME:-$HOME/.codex}/skills" --resources references
```
