...
fn setup_helper_embeds_as_invoker_manifest() -> Result<()> {
    let setup_executable = std::env::var_os("CARGO_BIN_EXE_hexa-windows-sandbox-setup")
        .or_else(|| std::env::var_os("CARGO_BIN_EXE_hexa_windows_sandbox_setup"))
        .map(PathBuf::from)
        .or_else(|| option_env!("CARGO_BIN_EXE_hexa-windows-sandbox-setup").map(PathBuf::from))
