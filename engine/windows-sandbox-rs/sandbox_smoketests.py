...
# Run a suite of smoke tests against the Windows sandbox via Hexa Engine
...
def _resolve_codex_cmd() -> List[str]:
    """Resolve the Hexa Engine/upstream CLI binary used to invoke `hexa-engine sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix used to run the Hexa Engine-compatible CLI.
    """
    ...
    raise FileNotFoundError(
        "Hexa Engine-compatible CLI not found. Build it first, e.g.\n"
...
def run_sbx(
    ...
    # Map policy to Hexa Engine CLI overrides.
...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            ...
            proxy_env = {
                "HEXA_ENGINE_HOME": str(proxy_home),
            ...
            rc_direct, _out_direct, err_direct = run_sbx(
                ...
                direct_cmd,
                WS_ROOT,
                env_extra={"HEXA_ENGINE_HOME": str(proxy_home)},
            )
    ...
    codex_home = Path(os.environ["USERPROFILE"]) / ".hexa"
