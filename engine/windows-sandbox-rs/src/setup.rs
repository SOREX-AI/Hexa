...
pub const OFFLINE_USERNAME: &str = "HexaSandboxOffline";
pub const ONLINE_USERNAME: &str = "HexaSandboxOnline";
const ERROR_CANCELLED: u32 = 1223;
const SECURITY_BUILTIN_DOMAIN_RID: u32 = 0x0000_0020;
const DOMAIN_ALIAS_RID_ADMINS: u32 = 0x0000_0220;
const SETUP_EXE_FILENAME: &str = "HexaSandboxSetup.exe";
...
pub fn run_elevated_provisioning_setup(
        ...
        )
    {
        return Err(failure(
            SetupErrorCode::OrchestratorSandboxDirCreateFailed,
            format!(
                "sandbox provisioning HEXA_ENGINE_HOME must be an absolute local disk path: {}",
                codex_home.display()
            ),
        ));
    }
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, codex_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to HEXA_ENGINE_HOME or anything under HEXA_ENGINE_HOME/.sandbox,
    // HEXA_ENGINE_HOME/.sandbox-bin, or HEXA_ENGINE_HOME/.sandbox-secrets. These locations contain sandbox
...
mod tests {
    ...
    fn setup_exe_lookup_checks_package_resource_dir_for_bin_exe() {
        ...
        let setup_exe = resources_dir.join("HexaSandboxSetup.exe");
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        let extra_codex = extra_write_root.join(".hexa");
