...
pub(crate) fn spawn_runner_transport(
    ...
    let runner_cmdline = runner_exe
        .to_str()
        .map(str::to_owned)
        .unwrap_or_else(|| "HexaCommandRunner.exe".to_string());
    ...
    if let Err(err) = connect_result {
        unsafe {
            ...
            // that child instead of leaking a stray `HexaCommandRunner.exe`.
            if pi.hProcess != 0 {
    ...
    let startup_result = (|| -> Result<()> {
        ...
        // runner instead of leaving a stray `HexaCommandRunner.exe` behind.
