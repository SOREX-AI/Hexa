...
pub(crate) const RESOURCES_DIRNAME: &str = "hexa-resources";
...
impl HelperExecutable {
    fn file_name(self) -> &'static str {
        match self {
            Self::CommandRunner => "HexaCommandRunner.exe",
        }
    }
...
mod tests {
    ...
    fn copy_runner_into_shared_bin_dir() {
        ...
        let runner_source = source_dir.join("HexaCommandRunner.exe");
    ...
    fn helper_source_lookup_checks_resource_dir() {
        ...
        let helper = resources_dir.join("HexaCommandRunner.exe");
        fs::write(&exe, b"codex").expect("write exe");
        fs::write(&helper, b"runner").expect("write helper");

        let resolved =
            bundled_executable_path_for_exe(&exe, /*file_name*/ "HexaCommandRunner.exe")
                .expect("helper path");
    ...
    fn helper_source_lookup_resolves_bin_junctions() {
        ...
        let helper = resources_dir.join("HexaSandboxSetup.exe");
        ...
        assert_eq!(
            bundled_executable_path_for_exe(
                ...
                /*file_name*/ "HexaSandboxSetup.exe"
            ),
    ...
    fn helper_source_lookup_prefers_package_resource_dir_over_bin_resource_dir() {
        ...
        let exe = bin_dir.join("codex.exe");
        let package_helper = package_resources_dir.join("HexaCommandRunner.exe");
        let bin_helper = bin_resources_dir.join("HexaCommandRunner.exe");
        fs::write(&exe, b"codex").expect("write exe");
    ...
    fn helper_source_lookup_prefers_direct_sibling_over_resource_dir() {
        ...
        fs::create_dir_all(&resources_dir).expect("create resources dir");
        let exe = release_dir.join("codex.exe");
        let sibling_helper = release_dir.join("HexaCommandRunner.exe");
        let resource_helper = resources_dir.join("HexaCommandRunner.exe");
        fs::write(&exe, b"codex").expect("write exe");
    ...
    fn materialized_file_name_adds_suffix_before_extension() {
        ...
        assert_eq!(file_name, "hexa-command-runner-test-suffix.exe");
    }
}
