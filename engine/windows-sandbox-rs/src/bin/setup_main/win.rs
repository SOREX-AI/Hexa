...
fn lock_sandbox_dir(
    ...
    // ProvisionOnly accepts another user's HEXA_ENGINE_HOME; keep its ACL mutation
...
pub fn main() -> Result<()> {
    ...
    if let Err(e) = &ret {
        ...
        if let Ok(codex_home) = std::env::var("HEXA_ENGINE_HOME") {
...
fn run_setup_full(payload: &Payload, log: &mut dyn Write, sbx_dir: &Path) -> Result<()> {
    ...
    for path in &payload.deny_write_paths {
        ...
        // legacy protected children such as `.git`, `.hexa`, and `.agents`.
        //
    ...
    // Elevated provisioning already protects the helper directory before the
...
mod tests {
    ...
    fn payload_json() -> serde_json::Value {
        json!({
            ...
            "offline_username": "HexaSandboxOffline",
            "online_username": "HexaSandboxOnline",
    ...
    fn deny_path_includes_nested_active_root_sid() {
        ...
        let codex_home = temp.path().join("codex-home");
        let workspace = temp.path().join("workspace");
        let protected_dir = workspace.join(".hexa");
