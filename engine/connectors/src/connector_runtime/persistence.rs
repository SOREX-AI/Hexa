...
fn write_cached_codex_apps_server_info<T: ConnectorRuntimePayload>(
    ...
    .context("failed to serialize Hexa connected apps server info cache")?;
    ...
    T: ConnectorRuntimePayload,
{
    ...
    if let Err(err) = &server_info_result {
        tracing::warn!("failed to write Hexa connected apps server info cache: {err:#}");
    }
...
fn write_codex_apps_cache_file(
    ...
    let parent = cache_path.parent().ok_or_else(|| {
        anyhow!(
            "Hexa connected apps {cache_name} cache path `{}` has no parent",
            cache_path.display()
        )
    })?;
    std::fs::create_dir_all(parent).with_context(|| {
        format!(
            "failed to create Hexa connected apps {cache_name} cache directory `{}`",
            parent.display()
        )
    })?;
    let mut temporary = NamedTempFile::new_in(parent).with_context(|| {
        format!(
            "failed to create temporary Hexa connected apps {cache_name} cache in `{}`",
            parent.display()
        )
    })?;
    temporary.write_all(&bytes).with_context(|| {
        format!(
            "failed to write temporary Hexa connected apps {cache_name} cache for `{}`",
            cache_path.display()
        )
    })?;
    temporary.persist(cache_path).map_err(|error| {
        anyhow!(
            "failed to atomically replace Hexa connected apps {cache_name} cache `{}`: {}",
            cache_path.display(),
            error.error
        )
    })?;
