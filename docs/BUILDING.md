...
1. Update `shell/package.json` → `version`. This is the packaged Electron application version and determines installer artifact names such as `Hexa-0.0.1-x64.exe`.
...
| `node hexa.mjs upstream-update <ref>`   | Preview an engine + full upstream-reference update; add `--apply` to install it |
| `node hexa.mjs upstream-clean`          | Remove ignored upstream staging, reference, and rollback trees              |
...
The guarded upstream updater installs `codex-rs/` into the tracked `engine/` tree after applying Hexa's permanent adapter. Every other upstream repository file is mirrored temporarily under the ignored `vendor/upstream-engine-reference/` review tree, excluding `.git/`; `UPSTREAM_SOURCE.json` records the remote, requested ref, and resolved commit. Previous trees are retained in ignored `*.before-update/` rollback directories until validation is complete. Run `node hexa.mjs upstream-clean` after validation and before committing so none of those disposable trees are uploaded. Use `--reference-only` only to synchronize the local reference snapshot to an engine revision that was already installed separately.
