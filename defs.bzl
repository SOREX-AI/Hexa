...
def codex_rust_crate(
    ...
    if manifest_relpath.startswith("engine/"):
        manifest_relpath = manifest_relpath[len("engine/"):]
    ...
    if lib_srcs:
        ...
        rust_test(
            ...
            # tests under gnullvm.
            # Bazel has emitted both `engine/<crate>/...` and
            # `../engine/<crate>/...` paths for `file!()`. Strip either
            # prefix so the workspace-root launcher sees Cargo-like metadata
            # such as `tui/src/...`.
            rustc_flags = rustc_flags_extra + WINDOWS_RUSTC_LINK_FLAGS + [
                "--remap-path-prefix=../engine=",
                "--remap-path-prefix=engine=",
            ],
            rustc_env = rustc_env,
        ...
        workspace_root_test(
            ...
            workspace_root_marker = "//engine/utils/cargo-bin:repo_root.marker",
            tags = test_tags,
            **unit_test_kwargs
        )
    ...
    for test in native.glob(["tests/*.rs"], allow_empty = True):
        ...
        if run_tests_with_wine_exec:
            ...
            foreign_platform_binary(
                ...
                binary = "//engine/exec-server/testing:exec-server",
            ...
            workspace_root_test(
                ...
                test_bin = "//engine/exec-server/testing:wine-exec-test-runner",
                workspace_root_marker = "//engine/utils/cargo-bin:repo_root.marker",
