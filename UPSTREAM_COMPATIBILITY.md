...
- The complete non-`codex-rs` upstream repository snapshot may be staged under the ignored `vendor/upstream-engine-reference/` review tree. It is disposable, excludes Git metadata, records its exact source in `UPSTREAM_SOURCE.json`, and is removed before commits with `node hexa.mjs upstream-clean`.

## Update guarantee

`shell/scripts/update-upstream.mjs` stages the engine and complete non-engine upstream reference from one resolved commit, validates and patches the engine before installation, and retains ignored rollback trees. `shell/scripts/engine-patches.mjs` is the permanent downstream adapter. For a raw upstream engine snapshot it applies the Hexa integration changes: product-facing branding, local-model compatibility, runtime/state isolation (including Hexa home/project/resource/socket namespaces), and self-updater removal. It then invokes `shell/scripts/engine-package-rebrand.mjs` to transform Cargo package/bin identities and selected source directories in the downstream copy. A second pass must be a no-op.
